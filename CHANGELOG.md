...
- **未由验证存档方完成的一项**：指挥师要求复核`enterprise_password_manual_refresh`与`daily_role_headcount_snapshot`在**生产**`users.db`的行数，以确认「两表为空、无需数据迁移」这一判断。验证存档方**没有服务器访问权限**（手册一.5「云服务器现场操作」），无法执行，已交还指挥师。本机`./data/users.db`只读查得两表均为0行——那正是实施方的依据本身，不能替代生产实测。**在生产行数确认之前不得视为已验证**。
