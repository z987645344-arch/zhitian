...
## 2026-08-16 v3.4.1 与 v3.4.2 文档补丁存档：协作架构交接与服务商记载校正

- **补记 v3.4.1（2026-08-15）**：项目协作架构由「Claude对话式指挥＋Codex执行＋WorkBuddy测试」切换为「Claude Code指挥＋按任务性质调度Codex或Claude Code执行」；历史架构决策与事故记录归档至`docs/history/`；README与运维文档同步校准。该存档当时未追加本文件条目，此处补记以恢复标签与改动记录的连续性。
- **v3.4.2 工作手册**：新增「云服务器现场操作由指挥师独占执行」——两个执行agent均不具备服务器访问能力，SSH登录后的一切动作不写进给它们的指令；受版本控制的部署文件改动仍归执行agent。附7条来自真实事故的操作红线（禁止输出完整`docker compose config`、不擅改服务器`.env`、`up -d`不重建镜像、备份恢复需`--confirm-service-stopped`、禁止整目录拷贝、不可逆操作逐次确认、本机结果不冒充生产）。
...
## 2026-08-17 手册补存档「默认归属」，起因是知了hub的一次真实越权（`v3.4.4`，仅文档）

- **改动来源不是本项目出的错，是规则不对称的另一侧。** 知了hub `v2.1` 的提交、推送与打标签由指挥师本人执行，而用户此前已明确指示交给执行agent；用户随后说的“一起存档吧”回答的是批次问题（三次授权是否合并），被误读为改派。追因时发现「存档职责划分」当时**只写在本仓库手册、未同步到知了hub**——本轮开工前实测确认该关键词知天1处、知了hub 0处。
- **本仓库这一侧同样有缝，本轮补的正是它。** 原措辞「推送执行由用户逐次指定」**没有默认归属**，照它办指挥师自行推送算不上违规。现改为「**默认由执行agent完成**」，与一.5章开头「Claude Code执行者更适合GitHub操作」的取向一致；用户可以逐次改派，但**必须明确指定执行者**，对**批次或时机**的回答不构成改派。原有的「三次独立授权、任何一方不得自主commit」与「不要把存档整体收归指挥师」两条保留不变。
- **新增一条跨项目规则**：凡是需要两个项目共同遵守的协作规则，必须同时写进两份`claude_skill.md`（知天与知了hub），只写一份不算已确立。该结论与失误实例写在同一处，避免日后只看到规则、看不到它为什么存在。
- **范围与边界**：只改`docs/claude_skill.md`的「存档职责划分」一节及文档头部「最后更新」日期，实测共10行新增、3行删除；无代码、接口、数据结构或部署行为变化，未写入任何域名、IP或凭据。按三段式补丁号存为`v3.4.4`，与知了hub侧的`v2.1.1`同期存档，两份手册的该节措辞自此对齐。

## 2026-08-17 v3.4.5：工作手册瘦身为执行agent视角，补齐存档与compose约束

- **移出指挥师侧内容，章节编号保留历史值**：`docs/claude_skill.md`由302行降到203行。删除第一章的指挥师职责7条与4条加粗原则、一.5章的Codex/Claude Code取向与选型要求、第二章0–9步工作流程、第三章「执行agent指令格式」整章（指令骨架、格式要点、指令范例、指令注意事项），并把「云服务器现场操作」由「指挥师独占＋7条服务器红线」缩写为「不归你＋边界」两段；被删内容已完整移到仓库外`指挥师手册.md`，不是丢弃。删除后出现「二 → 四」空档，刻意不重排、不复用旧号——编号被本文件与仓库外文档交叉引用。原第三章的「CHANGELOG.md条目格式规范」5条正文逐字搬到新**第八章**；新增**第九章存档操作规范**（6条，对齐知了hub同名章节——那边一直有、知天一直缺，属规则单边缺失。两处有意差异：知了hub版第4、6条的「CI触发／CI链接」因知天无对应CI而删去）。
- **新增「本机运行compose的约束」一节，来源是执行者的跨项目上报**：原7条服务器红线中的第1条（禁止输出完整`docker compose config`——含真实`env_file`时会展开全部密钥，2026-08-13已真实误展开并触发密钥轮换；用`--format json`再管道过滤同样不行，密钥已进入进程输出）与第3条（`up -d`不重建已有标签镜像、也不因挂载内容变化重建容器，「容器healthy」不能单独作为版本正确的证据）实际约束的是「跑compose」这个动作、与在哪台机器上跑无关，随整节移出后在执行者手册中失去落点。该缺口由本项目执行者在瘦身当轮上报，**知了hub同轮同步补入同名章节**。
- **修三处失效引用与框架句**：①「存档职责划分」表内「与本章开头…取向一致」改指第九章，原引用的开头段已被本轮删除；②第五、六章开头的「指挥师写指令时…」改为面向执行者的「以下约束都是本项目真实踩过的坑，动相关代码前先看一遍」与「改动前先按下表确认落点」——两句写在必须照做的章节开头却是指挥师视角，读者会误以为整章与己无关；③第七章开头引用的「指令第5步」随第三章删除而失效，改为「派给你的指令通常会在收尾步骤要求你更新」。
- **补回一条误删规则、对齐三条单边规则**：①**「开工前的 Git 检查」**原是二章第0步的强制规则、被本轮瘦身删除——当时判断「不会丢失，会出现在每条派给你的指令第0步」，实为「靠人记得、不靠手册」，正是`structure.md`点名的失效模式；指挥师核实后发现两份手册均已缺失，规范正文由指挥师直接写入两个项目并经哈希比对确认逐字节一致（差异仅项目参数引用块）。②对齐三条此前只存在于知了hub手册的执行者原则——不可逆操作前先只读核对、人工步骤不代报通过、你的自检不构成验收，追加到一.5「以下原则不因任务类型变化而改变」清单末尾（4条→7条）；其中「自检不构成验收」是本次瘦身补丁漏派、另两条为历史单边，由指挥师跨项目核对两份手册后发现，前两条按知天数据面调整了举例（`users.db`/`history.db`/Chroma与备份恢复脚本、真实DNS与证书签发）。
- **只有一条规则被裁定作废**：原第三章「范围明确：指令里没提的就是不动的」按用户2026-08-17裁定改采相反口径（禁区必须显式写出），已随整章删除、全文0命中。除此之外规则内容无一条被修改。保留段按行区间做SHA-256比对：「文档权威性原则」「主机名的公开边界」「第七章5条编号正文与末尾原则句」「CHANGELOG 5条正文」四段与`HEAD`逐字一致；「存档职责划分」与「四/五/六章」两段的差异经逐行比对恰好等于上述获准改动的1处与2处，无附带改动。

## 2026-08-17 更正`claude_memory.md`中已过时的「当前等待」（随`v3.4.5`同批提交）

- 「当前等待」原写着部署仓库「工作区现有7项未提交改动……最后再升级`v3.4`」，而这些改动已于2026-08-16存档并推送：部署仓库`v3.4`（提交`8585c7d`）、后端`v3.4.3`（提交`71a1882`），两个仓库的远程分支、标签与本地HEAD三者一致、工作区归零。
- 按「文档权威性原则」以真实状态为准改写为：仓库侧已完成，剩余全部为服务器侧待办——签发Origin CA证书、添加两条DNS记录、现场填写`.env`（含`ZHITIAN_FORCE_HTTPS=on`，私钥须`chown root:101`+`chmod 0640`）、重建反代容器、完成线上验证并更新生产`CORS_ORIGINS`；并保留「服务器当前仍检出`v3.3`，升级必须显式切换标签」。
- 与手册瘦身无关，故单列一条，避免将来查历史时被埋进瘦身条目；本轮未改`claude_memory.md`其他内容。

## 2026-08-18 手册补齐三条只写在知了hub的规则，并改用「攒够再打标」的节奏

- **新增 8.1「版本号与标签语义」**：两段式功能版本 + 三段式文档补丁号的判定口径，第2、3、4条与知了hub 8.1逐字相同；另写明四个仓库各自独立打标、后端与`zhitian-deploy`同到`v3.4`属巧合不构成约定。该语义此前只落在知了hub 手册，而知天**实际一直在用**（`v3.4.1`～`v3.4.5`共5个补丁号），属规则单边、靠人记得。
- **新增 8.1 下「什么时候才打标签」**：打标不再逐轮——纯文档/流程整理累积若干轮后合并打一个三段式，真实行为变化、主题收尾或服务器部署前才打。起因是四天内（08-15～08-18）打出9个三段式补丁标签（知了hub 4个、知天5个）全为文档整理，没有一个标记出真正不同的状态，而标签一经推送不删不重打，噪声会永久留在列表里。
- **八章新增第6条**：区分「存档条目」与「普通工作条目」，后者不写版本号、不暗示已提交。该区分知了hub 八章本来就有、知天缺；且此前每轮都被当成存档条目处理，属规则在手册里、执行时没照做。**本条目本身即按新口径写成，不含版本号。**
- **新增第十章「部署验证四层」**：静态审阅 / 配置解析 / 本地容器 / 真实部署，报告必须写清完成到哪一层。对应知了hub 7.4，第1–3层基本逐字相同，第4层按知天部署面（双子域名、Chroma）调整并注明「这一层不归执行agent」。此前知天只有红线里一句「本机验证结果不得冒充生产结果」，缺分层与硬性报告要求。`docs/claude_skill.md` 203行→248行，零代码。
- **`claude_memory.md`两格更正（与手册整改无关）**：「当前等待」原列的Origin CA签发、两条DNS记录、现场`.env`、反代容器重建、生产`CORS_ORIGINS`五项，据指挥师回报已于2026-08-16在服务器完成，属过期记录，按文档权威性原则改写为真实状态（真正剩下的是服务器按标签升级）；「上一轮完成」同步。执行者另更正该格中`claude_skill.md`行数笔误：原写230行，实测248行（203+45，与`git diff --numstat`的46增1删一致）。

## 2026-08-18 三个仓库补 `.gitignore` 的 `.env` 衍生文件规则

- **起因是一次真实暴露风险**：2026-08-16 指挥师在服务器改配置时于仓库目录内建了 4 个 `.env` 备份（含真实 DeepSeek/Tavily/阿里云密钥）。本仓库当时只有 `.env` 一条规则，`.env.bak-1` 这类衍生名**不被忽略**，而本仓库是公开仓库，一次 `git add .` 即可推上去。那 4 个文件已移出仓库，本轮修的是根因。
- **改法统一为 `zhitian-deploy` 的三行写法**：`.env` + `.env.*` + `!.env.example`。本仓库在原第 1 行 `.env` 紧下方补后两行；`zhitian_app` 在其第 27 行下同样补两行；`zhitian_admin` 三行一起加——**实测发现它此前连 `.env` 本身都不挡**，是本轮才暴露的新问题。三处均**不做路径锚定**，因此在任意子目录同样生效（实测 `docs/.env.bak-1`、`web_client/.env.local` 均命中 `.env.*`）。
- **判断口径改为实测**：忽略规则一律以 `git check-ignore` 为准，不再靠读 `.gitignore` 推断。改后四仓库 × 四文件全部实测：`.env`/`.env.bak-1`/`.env.local` 均被忽略，`.env.example` 均**不被忽略**。
- **`check-ignore` 的一个坑**：`.env.example` 在本仓库与 `zhitian-deploy` 返回退出码 1（无匹配），在 `zhitian_admin`/`zhitian_app` 却返回 0 并显示 `!.env.example`——差异只因前两者的 `.env.example` **已被跟踪**而 `check-ignore` 默认跳过已跟踪文件，加 `--no-index` 后四者一致。另用落盘实测确证：临时建 `.env`/`.env.bak-1`/`.env.example` 后，前两者进 `!!` 忽略清单、后者为 `??` 未跟踪，测试文件已删除。
- 三个仓库各自只增 2–3 行，未删除或改动任何既有规则；`zhitian-deploy` 本轮不改，只核对其三行仍齐全。本轮零代码改动，不打标签。

## 2026-08-18 手册新增第十一章「判断文件是否被 Git 忽略」

- 把上一轮 `.gitignore` 整改中踩到的 `git check-ignore` 三个坑写成规则：①**加 `-v` 后只要有规则命中就返回 0，包括 `!` 开头的否定规则**，因此 `-v` 只能看命中了哪条、不能用退出码判断；②判断结果用 `git check-ignore -q`，0=被忽略、1=未被忽略；③已跟踪文件默认被跳过、永远返回 1，要测必须加 `--no-index`。并要求拿不准时以未跟踪探针文件加 `git status --ignored` 实测为准。
- 章末写入硬约束：**在仓库目录内创建任何含密钥的文件（备份、临时副本、导出）之前，先按本章方法确认它会被忽略**——2026-08-16 服务器上那 4 个含真实第三方密钥的 `.env` 备份，正是在只挡精确 `.env` 的规则下产生的。
- 该章由指挥师直接写入两个项目以保证正文逐字相同；来源是两个项目的执行者在同一轮**各自独立**踩到第 1、3 条，且都改用落盘探针证伪了原指令「`.env.example` 应当退出码 1」的说法。
- 执行者复核了三条断言，其中此前未验过的第 ② 条本轮补测：`zhitian_admin` 中 `-q .env.example` 返回 1、`-q .env.bak-1` 与 `-q .env` 返回 0；`zhitian` 中已跟踪的 `.env.example` 不加 `--no-index` 返回 1，加了之后命中 `!.env.example` 且 `-q` 仍返回 1。三条与正文一致。
- `docs/claude_skill.md` 248 行→263 行，改动仅头部第 6 行与新增第十一章；六个「保留」段与八/九/十章整段经 SHA-256 比对与上次提交逐字一致。零代码改动，不打标签。

## 2026-08-18 放宽 `.gitignore` 的模板否定规则为 `!.env*.example`

- **上一轮补的规则有个静默缺陷**：`!.env.example` 只放行恰好叫这个名字的文件，`.env.local.example`、`.env.production.example` 这类模板会命中 `.env.*` 被忽略——**提交时静默排除，文件永远进不了仓库，而 diff 与 CI 都不报任何异常**。本仓库改为 `!.env*.example`，只动这一行，1 增 1 删。
- **发现途径是另一个项目的真实事故**：知了hub 执行者做本机 Compose 验证时新建 `.env.local.example`，差点无声消失；该仓库已先行放宽并验证。按两份手册末尾「凡两个项目共同遵守的规则必须同时写进两份」，知天四个仓库同批跟上。
- **落盘探针实测（本仓库 7 项全过）**：`.env`、`.env.local`、`.env.bak-1`、`.env.production`、`docs/.env` 均归 `!!` 被忽略；`.env.local.example` 归 `??` 可被 `git add` 看见；已跟踪的 `.env.example` 仍是已跟踪状态、无改动、`--no-index` 下 `-q` 返回 1（未被忽略）。实测口径按手册第十一章：不看退出码、用未跟踪探针文件加 `git status --ignored` 落盘判定。
- 探针过程中真实的 `.env`（未跟踪、含密钥）与已跟踪的 `.env.example` 全程未被创建或覆盖，探针按「内容恰为 PROBE=1」精确清理，清理后本仓库只剩 `.gitignore` 一处改动。
- 零代码改动，未运行 `run_tests.bat`；按 8.1 不打标签，本条为普通工作条目。

## 2026-08-18 手册新增一.5「改动超出指令范围时怎么办」

- **新增一节三档口径 + 一条确认送达规定**：①不改就交付不了——可以改，但同一轮显式报告改了什么、为什么、怎么回退；②放宽任何安全相关规则——即使实测证明防护未削弱也必须显式请求确认，**实测决定改动是否正确，不决定执行者有权替指挥师拍板**；③扩大授权范围、动未授权的仓库或文件——停下不改。另加：**指挥师的确认必须出现在发给执行者的指令文本里才算数**，只存在于指挥师与用户对话中的「我已确认」等同于没收到，应继续按未确认处理并追问。
- **来源是一次真实往返**：知了hub 执行者把 `!.env.example` 放宽为 `!.env*.example`（不改则新建模板文件被静默排除），并连续三轮记录「此事仍悬着」；指挥师每轮都在与用户的对话里确认过，但那句话从未出现在转发给执行者的文本中——三轮追问事后证明每轮都对。该口径此前只写在仓库外的指挥师手册，而它约束的是执行者行为。
- **验证用剔除法而非只看 diff**：把新增的第 47–61 行整段删掉后，文件与上一次提交**逐字节相同**（`cmp` 无差异，263 行完全吻合），因此确证是纯插入、其余各节一字未动；另用分段 SHA-256 交叉核对 7 段（含九/十/十一章整段）行号 +15 后哈希全部不变。结构检查：0 处连续空行、无孤立分隔线、无标题跳级、新节表格 2 列规整、UTF-8 无 BOM 纯 LF。
- **执行者本轮的两处判断**：①头部「最后更新」行漏列该节，已补为「一.5『开工前的 Git 检查』**与『改动超出指令范围时怎么办』**」——只动头部、不碰新节正文，补前补后新节正文 md5 均为 `8d5bc3ed7ed6b29e`，与另一项目的逐字节一致性未受影响；②新节末尾引用的 `v2.1` 越权在下一节「存档职责划分」才展开，属前向引用，**刻意不加「见下节」之类的位置指引**——指挥师说明两个项目该节所在位置不同，位置性交叉引用在另一边会失准。
- 与另一项目是否真的逐字相同**无法从本仓库单方验证**，需指挥师两边比对；本轮按四种合理边界给出本侧 md5 供核（第 47–61 行整块为 `8d5bc3ed7ed6b29e`，口径为 `sed | md5sum`、含末尾换行）。零代码改动，不打标签。

## 2026-08-18 手册新增跨项目边界与 `web_client` 前端规范

- **新增一.5「跨项目工作时的边界」**（含禁区目录、不要把一个项目的参数套用到另一个、存档仍按仓库分开三节）：起因是用户决定 Codex 工作区放在 `D:\zhiliao` 根目录、跨两个项目做设计。正文与知了hub 1.3 逐字节相同，按指挥师给的取值命令 `sed -n '64,90p' | md5sum` 实测为 `35e4cd9b85a603fb`，与预期一致。
- **新增第十二章「`web_client` 前端规范」**（12.1 架构事实 / 12.2 后端调用走 `js/api.js` / 12.3 样式变量优先 / 12.4 缓存击穿不一致 / 12.5 前端通用要求）。这是本轮最实质的一项：此前第四章编码规范 100% 是后端 Python，`web_client`、HTML、CSS、JavaScript 在全文 0 命中，而三个页面即将重做。
- **执行者逐条复核了章内全部数字，10 项中 9 项精确吻合**：三页 55/62/96 行、`chat.js` 624 行、`style.css` 471 行；三页加载顺序均为 css→`config.js`→`api.js`→页面脚本；`API` 确为全局 IIFE 单例且三页分别调用 2/4/10 处、4 处 `fetch(` 全在 `api.js` 内；`:root` 22 条自定义属性、全文 95 处 `var()`；`?v=` 确实只有 `chat.html` 有且同页两值（`css`与`api.js`一个值、`chat.js`另一个）。
- **更正 12.3 一处数字**：原写「约 30 处硬编码 hex 是历史欠账」——全文 hex 确为 30，但其中 **19 处是 `:root` 里的变量定义本身**，不是欠账也无法"收编进变量体系"。真正绕开变量的是 **11 处、3 个颜色**：`#fff` 5 处（与 `--surface: #ffffff` 同色属重复）、`#cbd9df` 5 处、`#eef3f4` 1 处，后两色根本没进 `:root`；另查出 9 处 `rgba()` 字面量同属此类，一并写入。改后条目给出颜色与处数，可直接定位。
- **另更正头部漏列并留一处待定**：头部「最后更新」只补列了第十二章、漏了「跨项目工作时的边界」，已补全（补后 md5 仍为 `35e4cd9b85a603fb`）。**未改但已上报**：跨项目边界的三个"子节"是 `###`，与父节及 `### 存档职责划分` 同级、结构上并非子节，修法是改为 `####`——因该三行落在跨项目 md5 校验区间内，按一.5「改动超出指令范围时怎么办」的口径交指挥师两边同步决定。范围验证用剔除法：删掉两个新增块后文件回到 278 行、与上次提交仅差头部与第 59 行两行。零代码改动，不打标签。

## 2026-08-18 手册新增两执行者分工，禁区扩为根目录白名单

- **新增一.5「两个执行者的分工：实施方与验证存档方」**：用户重组团队，每个项目两个执行者——实施方（通常 Codex）按指令改动并停在未提交状态，验证存档方（通常 Claude Code）独立验证后提交推送打标签。两条不可放宽：**两份记录不是一份**（验证方不复述实施方的话，用自己的方法重新证实并写实际验到的），**每轮写权限只归一人**（发现工作区有另一执行者的未提交改动即停下报告）。角色按轮次调换，以本轮指令写明的为准。
- **「跨项目工作时的边界」的禁区由单一目录扩为根目录白名单**：从原来只点名 `与指挥师的对话\`，改为列出 `D:\zhiliao\` 全部条目并分成"可动两个 / 其余一律不读不写"，另按三类风险分述（真实服务器凭据、历史版本会引入废弃实现且 `zhiliao1.4\` 带着别的项目的 agent 指令、无关项目与学习材料）。
- **执行者更正一处计数错误**：手册原称"根目录有 **13 个条目**"，实测为 **14 个**——差的是隐藏目录 `.workbuddy\`，`ls` 不加 `-A`、资源管理器默认设置下都看不到，正好显示 13，因此被漏数。它**两栏都没列到**，等于白名单开了个口子。已改为 14、把 `.workbuddy\` 补进禁止列，并在正文注明为何容易漏数。修正后用脚本双向比对：手册 2 可动 + 12 禁 = 14 项，与实际 14 项**双向零差异**（无实际有而手册没列的，也无手册列了而实际没有的）。
- **跨项目一致性**：按指挥师给的两条取值命令实测——分工节 `5b7a9eef63c6`、白名单节 `9780605fefe4`，**两项均与预期精确吻合**。其中白名单节因上述计数更正，现值变为 `59a96fa89b02`，**需指挥师同步到知了hub**（该目录是两个项目共用的同一个根，修正内容对两边完全相同）。
- **范围与结构**：剔除法核对——删掉新增的分工节后，与上次提交的差异只剩头部第 6 行与禁区一节的改写，无任何附带改动；0 处连续空行、无标题跳级、两张新表列数规整、UTF-8 无 BOM 纯 LF。**未验证项**：`zhiliao1.4\` 是否真带着 `AGENTS.md` 与 `CLAUDE.md` **刻意没查**——核实它需要列出禁止目录的内容，而本节正是规定不读这些目录，遵守优先于求证。零代码改动，不打标签。

## 2026-08-20 `web_client` 全面靠拢知了hub视觉语言（实施方现场记录）

- **改了什么**：只改 customer 网页端的 `login.html`、`register.html`、`chat.html` 与 `css/style.css`。三页保留原信息结构和业务语义，迁入深灰明暗基调、低饱和灰绿主操作、40×40 正圆「知」标记、较克制的字体字重与圆角、统一的表单/按钮/分隔线/动效语汇；聊天区另以分层表面、82% 桌面消息宽度和 1.78 行高处理长时间阅读。未改任何 JavaScript、接口封装、运行时后端地址或错误文案。
- **颜色字面量先收编再换色**：改前全文计数与指令一致，为 30 处 hex + 10 处 `rgba()`；其中真正位于 `:root` 外的是 11 + 9 处。先将这些外部字面量合并收编，再统一替换 token；改后 `:root` 共 40 个自定义属性，`:root` 外 hex 0 处、`rgba()` 0 处。
- **缓存击穿统一口径**：三页的 `style.css`、`api.js` 与各自页面脚本全部统一为 `?v=visual-kinship-a-1`，不再出现登录/注册无参数或聊天页同页多值；`config.js` 仍不加版本参数，继续由既有运行时配置和 Nginx `no-cache` 口径负责更新。
- **实施方实测证据**：用本地静态服务与隔离 Chromium，在 1440×900、390×844 两组视口逐页真实加载，六种组合均 HTTP 200、页面宽度等于视口宽度、无控制台错误或页面脚本异常；390px 聊天侧栏实测可从屏外展开到 0–300px，输入区和模式切换保持可见。聊天样本的正文对比度实算为助手 11.34:1、用户 8.86:1，角色标记 4.69:1、输入提示 6.08:1；助手正文行高 26.7px。模拟 `prefers-reduced-motion: reduce` 后按钮过渡和动画时长均为 0.001s。
- **没验的边界**：上述只是实施方自检，不构成视觉验收；视觉方向仍交用户看实际页面决定。聊天渲染使用本地模拟的 customer 令牌、会话列表与历史消息，未连接真实后端，未实测登录、注册、验证码、流式回答、附件上传/下载、限流、认证失败与网络不可达链路；也未做 Safari/Firefox、真机、生产 Nginx/CDN 缓存或线上部署验证。应用内浏览器连接因当前环境的信任路径限制未建立，故改用本机隔离 Chromium 完成同视口检查。

## 2026-08-20 客户端视觉靠拢的独立验证（验证存档方记录）

- **静态计数用我自己的方法重算，四项全部吻合**：`css/style.css` 471→517 行；`:root` 块（第 5–46 行）内自定义属性 **40** 条；全文 `var()` 引用 **95→148** 处；`:root` **外**的颜色字面量 **hex 11→0、`rgba()` 9→0**。最后这组尤其可核——11 与 9 正是 2026-08-18 那轮我逐个列出的欠账（`#fff`×5、`#cbd9df`×5、`#eef3f4`×1，另 9 处 `rgba()`），本轮被清零，收编范围与当时点名的完全对上。缓存参数：三页共 9 处本地资源引用**全部**为 `?v=visual-kinship-a-1`，无一遗漏也无第二个值；`config.js` 三页均不带版本参数，与实施方说明一致。`web_client/js/` 与 `config.js` 的 `git diff` 命中 0、未跟踪新文件 0。
- **渲染实测走的是与实施方不同的工具路径**：实施方因信任路径限制改用本机隔离 Chromium，我用的是应用内浏览器 + `127.0.0.1:8899` 静态服务，视口取 **1280×900 / 390×844**（实施方为 1440×900），因此是独立复现而非复算。三页×两视口 **6 组合**：`documentElement.scrollWidth == clientWidth`、逐元素扫描越界元素数 **0**，无横向溢出。网络层：三页 HTML 与全部静态资源均 **200**（一次 CSS 重取为 304 缓存校验）。
- **对比度四项按 WCAG 公式实算，与声明精确一致**：助手 **11.34**、用户 **8.86**、角色标记 **4.69**、输入提示 **6.08**。取值方式是 `getComputedStyle` 读渲染后的前景色、逐级上溯取首个非透明背景色，再算相对亮度比，不是读 CSS 字面量。过程中一处自我纠正：我最初把「输入提示」当成 `<textarea>` 的 `placeholder` 属性，测得 2.77 与声明不符；查证后确认声明指的是 `.composer-hint`（`#composerHint` 那行可见文案），重测即得 6.08——**是我量错了对象，不是实施方报错**。聊天页需先种一个假 `zt_web_token` 才不被 `chat.js` 弹回登录页，气泡用页面自身类名注入（无后端无法产生真实消息），测完已移除并清空 localStorage。
- **一项我无法独立验证，如实标注**：`prefers-reduced-motion: reduce` 下的实际计算值。应用内浏览器不提供该媒体特性的模拟（会话中 `matchMedia('(prefers-reduced-motion: reduce)').matches` 恒为 false），因此**我做到的是规则层而非渲染层**：从 CSSOM 取出该媒体块，确认声明为 `transition-duration: 1ms`、`animation-duration: 1ms`（即 0.001s），且**四条声明均带 `!important`**，能压过全文唯一用类选择器声明过渡的 `.session-item`——否则 `*` 选择器特异性为 0 会被它盖掉。声明值与实施方所报 0.001s 自洽，但"模拟后实测"这一步我没做成。
- **新发现一处本轮引入的可访问性回归（未修，属实施方职责）**：`<textarea>` 的 placeholder **没有任何 `::placeholder` 规则**（全文 0 命中），落到浏览器默认色 `#757575`。它在旧主题的白色 `--surface: #ffffff` 上是 **4.61:1**（勉强过 AA），在新主题的深色 `--surface: #29343a` 上只有 **2.77:1**，**低于 WCAG AA 的 4.5:1**——暗色化时 placeholder 未被收编进 token 体系。另注：`login.html` 与 `register.html` 各有 **2 个 `<h1>`**，与第十二章 12.5「每页一个主 h1」不符，但 HEAD 版本同样是 2，**属既有问题、非本轮引入**。两项都只报告不改：本轮我是验证存档方，改前端是实施方的车道。

## 2026-08-20 修复暗色 placeholder 对比度并增加返回知了hub的独立入口（实施方现场记录）

- **修复上一轮暗色化引入的 placeholder 回归**：`css/style.css` 新增 `--placeholder: var(--subtle)`，`input::placeholder` 与 `textarea::placeholder` 只引用该 token 并显式 `opacity: 1`，没有在伪元素内写颜色字面量；`:root` 外 hex 与 `rgba()` 复核仍各为 0。Chromium 从聊天输入框读取到前景 `rgb(146, 159, 164)`、背景 `rgb(41, 52, 58)`，按 WCAG 相对亮度公式实算 **4.69:1**，由验证方发现的 2.77:1 恢复到 AA 4.5:1 以上。
- **三页采用同一套独立跨站机制**：`login.html`、`register.html` 的账号切换链接下方，以及 `chat.html` 的会话侧栏品牌区下方，均新增带 `CROSS-SITE` 标签的「返回知了hub主站」卡片，目标为公开主站、在新窗口打开且带 `rel="noopener noreferrer"`。圆形「知」标记不承担跨站跳转：登录/注册页仍回知天登录首页，工作台仍回知天工作台首页，链接各有明确可访问名称。未改现有两个 `<h1>` 的既有问题，也未改任何 JavaScript、接口封装或错误语义。
- **缓存击穿继续三页同口径**：三页的 `style.css`、`api.js` 与各自页面脚本共 9 处统一由 `?v=visual-kinship-a-1` 升为 **`?v=visual-kinship-a-2`**；运行时 `config.js` 仍不带版本参数。新增入口和样式均未引入构建链或第三方依赖。
- **实施方渲染证据**：本地静态服务 + 隔离 Chromium 实测 1440×900、390×844，三页 × 两视口共 6 组合全部 HTTP 200，`documentElement.scrollWidth == clientWidth`，无控制台错误、页面脚本异常或失败资源；三页外链均解析为同一公开主站地址，目标、`target`、`rel` 与可见尺寸全部符合预期。390px 工作台实点侧栏按钮后，抽屉范围为 0–300px，跨站卡片可见。
- **未验边界**：本记录是实施方自检，不构成视觉验收；未实际导航到生产主站，未验证生产 DNS/TLS、新窗口目标页或线上缓存更新。聊天页用本地模拟 customer 令牌与空会话响应渲染，未连接真实后端；未测 Safari、Firefox 或真机。`docs/claude_skill.md` 的 2 增 2 删是指挥师已授权保留的既有改动，本轮未编辑；提交、推送与预定 `v3.5` 标签均留给验证存档方。

## 2026-08-21 收编浏览器自绘界面的暗色配色（实施方现场记录）

- **浏览器原生配色进入同一 token 体系**：`css/style.css` 在 `:root` 声明 `color-scheme: dark`，新增 selection 前景/背景与 scrollbar 滑块/轨道四个语义 token；`:root` 自定义属性由 **40→45**。`::selection` 仅引用 token；滚动条分别在根节点和实际可滚动的 `.session-list`／`.chat-log`／`textarea` 两层收编，避免 `scrollbar-width` 不继承而让内部滚动区仍回退为 `auto`。
- **原生绘制实测**：应用内 Chromium 读取到 `:root`、`body`、密码输入框和隐藏文件输入框的 `color-scheme` 均为 `dark`。文本选择实测前景 `rgb(32, 41, 46)`、背景 `rgb(179, 183, 164)`，按 WCAG 相对亮度公式为 **7.21:1**；页面、会话列表、消息流和 textarea 的 scrollbar 均计算为 `rgb(146, 159, 164) rgb(32, 41, 46)` + `thin`，滑块/轨道对比度 **5.44:1**。
- **文件入口与滚动实测**：`chat.html` 的原生 `type="file"` 控件本来就带 `hidden`、计算值为 `display: none`，页面可见的是既有暗色自绘「附加文件」按钮；实点该按钮后真实触发单文件 `filechooser` 事件。另在 390×600 形成页面级纵向滚动，实测 `clientWidth == scrollWidth == 380`、无横向溢出，页面级细滚动条可见。
- **归零、缓存与视口核对**：`:root` 外颜色字面量复核为 **hex 0 / `rgba()` 0**；三页 `style.css`、`api.js` 和页面脚本共 9 处缓存参数统一推进为 **`?v=visual-kinship-a-3`**。本地静态服务下实测 login／register／chat 在 1440×900 与 390×844 共 6 组合均完成加载、根计算值为 dark，且 `scrollWidth == clientWidth`；未改上一轮跨站入口、placeholder 规则、任何 JavaScript 或接口封装。
- **未验证边界**：**`color-scheme: dark` 对 Chrome 自动填充背景的实际效果未验证——该步骤需要在真实浏览器触发密码管理器填充，已交回用户实测。** 文件选择器的操作系统对话框不属于页面渲染面，本轮只证实隐藏原生 input 继承 dark 且 chooser 事件被真实触发；未测 Safari、Firefox、真机、真实后端、生产 Nginx/CDN 或线上部署。本记录是实施方自检，不构成视觉验收；`docs/claude_skill.md` 的既有 2 增 2 删保持未动，提交、推送和预定 `v3.5` 标签交验证存档方。

## 2026-08-21 跨站入口与浏览器自绘界面收编的独立验证（验证存档方记录）

- **轮 A 三项自行复算，全部吻合**：跨站锚点 **3 条**（每页 1 条），`target="_blank"` 与 `rel="noopener noreferrer"` 各 3 处，**反向查「有 `_blank` 却无 rel」结果为 0**；品牌标记 1 处指 `./chat.html`、2 处指 `./login.html`，未被劫持；全 `web_client` 唯一外部主机是 `zhiliaohub.com`，**企业管理后台主机名 0 命中**。placeholder 在 chat 页 textarea 实测 fg `rgb(146,159,164)` / bg `rgb(41,52,58)` = **4.69**，与上一轮我测到的 2.77 基线对照，回归确已修复；login/register 输入框因背景更深（`rgb(23,32,37)`）实测 **6.08**——4.69 与 6.08 分属不同页面，两个数都对。
- **轮 B 的四组渲染数字逐项实测，与声明逐字一致**：`color-scheme` 三页 `:root` 与 `body` 计算值均为 **dark**（上一轮我测到的是 `normal`，正是本轮要补的缺口）；`::selection` 前景 `rgb(32,41,46)` / 背景 `rgb(179,183,164)`，按 WCAG 公式算 **7.21**；滚动条 `rgb(146,159,164)` / `rgb(32,41,46)` = **5.44**，且 `:root`、`.session-list`、`.chat-log`、`#messageInput` 四处计算值**均为 `thin`**——不只声明里说的"各容器"，根元素也收编了，页面级滚动条一并覆盖。
...
- **等价性不看声明看函数体**：把v3.6的`converter.py`与现在的`pdf.py`按缩进切出`_pdf_to_docx/_xlsx/_pptx`与`_to_docx/_to_xlsx/_to_pptx`逐行比对，差异只有三类且全部语义等价：六个第三方import提到模块级（已核实`fitz/pdfplumber/docx/openpyxl/pptx/pptx.util`确在`pdf.py`模块级）、docx里一个临时变量内联、pptx把`enumerate(pdf)`+`page_index + 1`改成`enumerate(document, start=1)`+`page_index`，产出的`page_N.png`文件名不变。首次比对用awk因正则报错两边都取到空文件、误报三个「完全一致」，已作废重做，此处记录以免后人复用该手法。
- **锁的粒度没被改松**：v3.6有`_conversion_lock`（LibreOffice）与`_pdf_conversion_lock`（PDF）两把。现在PDF那把改名`_pdf_processing_lock`搬进`pdf.py`并导出，`converter.py:28`用`get_pdf_processing_lock()`取回**同一个锁对象**绑到旧名字，新旧两条PDF路径共用一把；LibreOffice那把仍独立留在`converter.py:27`。`pdf_tools.py`前后都无锁，未变。
- **注册表裁决实测而非读码推断**：按生产入口`import main`后对10组CONVERT请求跑`registry.resolve()`。结果：`pdf→docx/xlsx/pptx`解到**pdf处理器**（`pdf.to_*`），`md/txt/docx/xlsx/pptx→pdf`与`pdf→pdf`解到libreoffice。两个处理器在`pdf→Office`上能力重叠（libreoffice声明`source_formats=["*"]`），`resolve()`按注册顺序取第一个`supports()`为真者，pdf处理器胜出——这与v3.6由`convert_pdf_to_office`专门处理PDF反向转换的分工一致，不是行为变化。验证途中曾疑`convert_file`传入带点的`.pdf`而`supports()`比不带点的`"pdf"`会导致永不命中，实测证伪：`models.py:59-62`的`field_validator`已统一去点转小写。
- **`generate_file`那个硬编码引擎标签是安全的**：`execution.py:874`写死`generation_engine="libreoffice"`。因该路径的源文件必为`.md`/`.txt`，实测这两种源在注册表下确实只解到libreoffice，标签与实际执行者一致；若将来放开源格式则此处会失真。
- **测试全部自己跑，未采信数字**：权威入口`run_tests.bat`得`442 passed, 5 deselected`（与实施方一致）；全量含integration得`447 passed, 0 failed`。分文件：契约层4、PDF处理器与pdf_tools共7、转换/文件存储/生成共37、Agent预算15。对照v3.6基线`428 passed, 5 deselected`（全量`432 passed, 1 failed`），净增14条用例且原先唯一的失败项已消除。
- **上一轮我报的过期断言确认修好且更严**：`test_converter_integration.py`不只把`success`改成`accepted`，还追加断言异步任务落到`done`且`result_doc_id`与`doc_id`一致；另用附加一字节的方式避开内容哈希去重。该文件实跑2 passed。
...
## 2026-08-22 后台入库段改为有界排队，同步段维持拒绝

- **两段策略刻意相反，理由不同**。同步段（转换、分块）维持「占不到立刻拒绝」：请求还挂着，等待会烧掉61秒响应预算，把一个人的洪水变成所有人的超时。后台段（向量化、Chroma写入）改为阻塞排队：响应早已返回`accepted`、用户在轮询`task_id`，没有响应预算可烧，上面那条反对理由在此不成立。`TASK_STATUSES`里的`pending`本就是为此准备的位置——等待期间任务保持`pending`，拿到槽位后才转`processing`，前端本就在轮询，能看到「排队中」。
- **后台段用独立的第二道信号量**（`MAX_CONCURRENT_INGEST_TASKS=2`），与同步段那把互不共用计数，已用`is not`断言与并发取样双向证实。队列深度上限`MAX_INGEST_QUEUE_DEPTH=8`：排队项持有的是已切好的`chunks: List[str]`（在内存里，单文档最多2000切片、源文件最大5MB），按最坏情形每项约10~15MB常驻，深度8约合80~120MB——在4核4G上与嵌入模型共存尚可，再深就是拿RAM换吞吐，而RAM正是嵌入模型要抢的东西。超深度时在返回`accepted`**之前**拒绝，实测被拒前后未终结任务数`0 -> 0`，证明拒绝早于建任务行。
...
## 2026-08-23 企业密码业务日边界改为显式UTC+8（实施方现场记录）

- **根因与修复边界**：生产容器默认UTC，而`enterprise_password.get_business_day()`原来依赖naive `datetime.now()`，使“凌晨4点”实际成为UTC 04:00（本地12:00）。现新增固定UTC+8业务时区，密码日、下一次刷新时间、邮件用量业务日窗口和每日人数快照统一复用该口径；没有设置容器`TZ`，验证码冷却、账号时间戳及其他naive时间逻辑均未改动。`next_refresh_at`现在携带明确`+08:00`偏移。
- **SQLite窗口保持兼容**：业务日展示窗口为本地`04:00+08:00`至次日`04:00+08:00`；用于查询既有UTC-naive SQLite时间戳时，单独转换为UTC-naive的`20:00`至次日`20:00`，避免把带偏移ISO字符串直接与历史无偏移字段比较。
- **存量数据结论**：只读核对本地真实`data/users.db`，`enterprise_password_manual_refresh=0`、`daily_role_headcount_snapshot=0`，无需本地迁移。即使生产存在历史行也不自动改写：旧UTC业务日的一条刷新计数可能横跨新口径的本地04:00，聚合行无法可靠拆成两个日期；历史人数快照也应保留其生成时口径。部署切换后从新业务日开始使用新键，避免用猜测迁移制造错误计数。
- **自动化证据**：显式UTC用例证明UTC 20:00（本地04:00）前后密码变化、UTC 04:00（本地12:00）前后密码不变，并断言无参路径向`datetime.now()`显式传入UTC+8；邮件统计用例核对本地04:00窗口准确映射为UTC 20:00。相关针对性回归`60 passed`，权威回归`469 passed, 5 deselected`，高于v3.9的467基线。
- **尚待验证存档方补验**：本Codex会话没有Docker CLI，未冒充完成容器内时钟实测；需由有Docker环境的验证存档方在UTC容器内补验当前区间与跨边界密码，确认运行态与上述自动化证据一致。

## 2026-08-23 定时备份改为显式UTC+8每日零点触发（实施方现场记录）

- **从漂移间隔改为固定时钟**：移除`SCHEDULED_BACKUP_INTERVAL_SECONDS`，新增严格`HH:MM`格式的`SCHEDULED_BACKUP_LOCAL_TIME`，默认`00:00`。调度器使用代码内显式`UTC+08:00`固定时区计算，不读取容器默认TZ；因此本地零点准确对应UTC 16:00，不会在UTC容器中误落到本地08:00。
- **跨重启判据改为本地业务日边界**：调度器只检查最新`zhitian-scheduled-backup-*`归档是否位于当前有效的UTC+8触发边界之后。同一本地日内重复重启不会重复生成；错过当日时刻且没有当日归档时立即补跑；目录为空仍立即产出第一份，避免新部署最长24小时无备份。归档前缀隔离、AES-256-GCM格式、手工备份和保留3份策略均未改变。
- **验证证据**：真实加密归档用例证明空目录立即产出；同一UTC+8日模拟三次重启后仍只有1份，时间推进到UTC 16:00（UTC+8次日00:00）后产出第2份；独立1秒边界断言证明15:59:59 UTC尚差1秒、16:00 UTC立即到期。调度/备份恢复/lifespan针对性回归`14 passed`，权威回归`471 passed, 5 deselected`，高于要求的467基线。
- **部署验证边界**：部署仓库已把Compose注入项同步为`SCHEDULED_BACKUP_LOCAL_TIME=00:00`，但本Codex会话中`docker`不在PATH，未冒充完成`docker compose config --quiet`或容器运行验证；本轮明确不部署，生产`.env`未修改，Compose解析由有Docker环境的验证存档方补验。

### 我怎么证实的（验证存档方）

- **存档前例行核对（手册九.1）**：两仓合计IPv4字面量0处、密钥凭据形态0处、服务器绝对路径0处、二进制0处；`zhitian_admin`（`90fb107`）与`zhitian_app`（`1a3a805`）均0处改动，无跨仓库误改。本仓无未跟踪文件；部署仓库唯一的未跟踪文件是本轮新增的`日常验证MVP.bat`，实测GBK可解码、非UTF-8、无BOM、CRLF 160/160，与既有两个批处理的编码约定完全一致。
- **CHANGELOG按要求保持分轮**：提交合并为每仓一次，但条目未合并——本仓两条（业务日边界、备份零点），部署仓库两条（v3.4.3验证入口、备份零点），三轮跨两仓分布。
- **存量数据结论（生产实测后的准确版本，取代此前「两表为空」的表述）**：指挥师在生产`users.db`只读确认——`enterprise_password_manual_refresh`为**0行**；`daily_role_headcount_snapshot`为**3行**，其中**2行**的`snapshot_date`是按旧UTC边界标注的（`created_at`落在UTC 02:40与03:27，即本地10:40与11:27；旧规则按UTC 04:00翻天，把它们归到了前一日），切换到显式UTC+8后存在**一次性口径不连续**。
- **经评估不迁移，理由不是「两表为空」——那对生产不成立**，而是三条：①重算不会主键冲突（08-15→08-16、08-21→08-22均不存在，今日08-23也不撞）；②值本身是测试期数据（`0,0,0,0`与`1,1,1,0`）；③为3行改动生产库，收益与风险不成比例。
...
## 2026-08-26 修复测试配置隔离与容器CI必填凭据缺口

- **测试不再依赖未跟踪`.env`**：`tests/conftest.py`补齐测试专用`DEEPSEEK_API_KEY`，与既有JWT、企业密码种子、个人Key加密密钥共同构成测试所需的完整强制配置。`ci.yml`继续显式传空DeepSeek Key，作为干净CI环境不借用外部凭据的反向门禁。
- **容器应用门禁恢复导入条件**：`container-ci.yml`的应用导入和真实`/ready`容器均注入固定测试用个人Key加密密钥，不写入镜像、不使用真实凭据，使后续pip-audit与Trivy不再因`config.py`导入提前失败而被跳过。
- **核心隔离实证**：本机临时移走`.env`并确认原路径不存在后，从权威入口执行得到`472 passed, 5 deselected, 0 failed`，随后在`finally`中原样恢复`.env`。这直接覆盖了此前“本机靠未跟踪配置全绿、CI连续11次失败”的根因，而不是只在富裕环境复测。
- **交付门禁升级为常设规则**：`docs/claude_skill.md`明确要求每轮同时报告本地权威回归、普通CI与后端容器CI的结论和运行链接；任一失败或未运行均不得宣称回归完成或进入打标。

## 2026-08-27 F38安全升级与容器漏洞可达性评估

- **先分类、后升级**：以容器CI运行[32987783153](https://github.com/z987645344-arch/zhitian/actions/runs/32987783153)上传的原始`pip-audit.json`和`trivy-image.json`为唯一基线，逐条核对392条Trivy记录。按知天实际调用路径归为：A类确认可达40条（全部是LibreOffice组件重复报告的`CVE-2012-5639`/`CVE-2018-10583`，严重度均为LOW，真实路径为用户Office文档进入headless LibreOffice）；B类条件可达63条（CRITICAL 2/HIGH 15/MEDIUM 22/LOW 21/UNKNOWN 3，主要是LibreOffice间接使用的GLib、libxml2、curl、TIFF等解析能力，只有特制输入进入对应子路径时才触达）；C类当前不可达289条（CRITICAL 4/HIGH 31/MEDIUM 103/LOW 148/UNKNOWN 3，包含Chroma服务端/RBAC三条、未执行的Perl及其他未调用子组件）。分类是风险优先级依据，不是从镜像中忽略或删除漏洞。
- **Chroma三条不升级**：项目在`layers/memory.py`与备份脚本中均使用嵌入式`chromadb.PersistentClient`，不启动Chroma HTTP服务，也不启用其多租户/RBAC或远程模型仓库端点；`CVE-2026-45830`、`CVE-2026-45831`、`CVE-2026-45833`因此归C类。`chromadb==0.5.0`当前没有包含修复的可用版本，且升级会牵动真实向量数据格式与检索链路，本轮按边界保留。
- **F38已安全修复**：`cryptography`从`48.0.1`精确升级到`50.0.1`。阿里云DirectMail传递依赖`alibabacloud-tea-openapi==0.4.5`在Python 3.9+声明`cryptography<49`，因此同步显式锁定已放宽到`cryptography<51`的`alibabacloud-tea-openapi==0.4.6`；`alibabacloud_dm20151123==1.11.0`保持不变，`pip check`无依赖冲突。项目实际使用AES-GCM，三条cryptography漏洞本身位于PKCS7/X.509路径、当前不可达，但升级消除了未来误用与安全关键依赖长期停留旧版的风险。
- **跨版本兼容证据**：在隔离目录中先用真实`cryptography==48.0.1`生成一条账号绑定的个人DeepSeek Key密文和一份`ZHITIAN-BACKUP-V1` AES-GCM归档，再升级到`50.0.1`；升级后个人密文可还原原值，旧归档可解密且manifest可读取。凭据、明文和密钥均未写入日志或提交。该证据验证格式/API向后兼容，但本轮Codex按协作边界没有SSH读取生产服务器上的3份归档或真实用户密文，生产实物复核仍须由有服务器权限的指挥师执行，不能用隔离证据冒充。
...
## 2026-08-27 Expert请求内超时熔断、结构化降级与网页执行动态

- **根因不是阶段预算简单相加，而是供应商失败没有传播**：项目原有`complex_deadline`继续作为唯一请求级总预算，`EXPERT_COMPLEX_TIMEOUT`默认120秒和生产90秒均未改动。此前第一次DeepSeek超时后，重排、搜索词改写、反思、输出观察和最终回答仍可能把剩余预算逐段用完；现把熔断状态放进同一个请求state，首次DeepSeek超时后跳过所有可选模型阶段、改写直接复用原问题，只允许一次最终回答机会，剩余预算不足时立即返回已有部分结果。`execution.py`中输出异常检查和搜索词改写两个原先直接领取独立全额timeout的漏网点也已接入剩余预算。
- **降级原因不再靠一句错误文案混合归因**：新增`web_provider_failed`、`web_no_results`、`query_rewrite_timeout`、`search_summary_timeout`、`document_rerank_timeout`、`final_answer_timeout`等结构化原因码；SSE结束前发送`request_status`，明确区分`completed/degraded/error`。搜索服务失败、无结果、结果已取得但模型整理超时、最终回答超时现在各自独立，不再都显示为“联网搜索遇到问题”。
- **执行动态来自真实工具边界且保持脱敏**：`search_documents`、`search_web`等工具在实际开始/结束位置发送`tool_status`，Pydantic载荷只允许工具标识、阶段、展示码、耗时、结果数和受控原因码；API Key/Token、完整参数、查询原文、搜索结果、文档正文、系统提示词和隐藏推理均不进入事件。网页端显示紧凑执行时间线和中文降级提示，四页静态缓存参数统一推进为`expert-status-1`。
- **F44以保守证据门槛闭环**：只有“多条可信候选 + 重排成功 + 原始分数明显高于阈值 + 重排分数足够高”时，Expert才把本地证据视为充分并跳过联网。单条0.579召回明确判为不足、仍会联网，覆盖“喝酒开车会判刑吗”但库中仅有民事实务资料的真实反例；重排超时也不会被误当作充分证据。该规则裁决服务端能力，不用关键词硬编码语义。
...
## 2026-08-28 工作手册对齐 2026-08-28 新组织分工

- 本轮实质改动只有 `docs/claude_skill.md` 一个文件（12 增 8 删），对齐 2026-08-28 确立的新组织分工：**新增统筹师角色**并由其独占云服务器与部署；**指挥师改为每项目一名**，只做确认问题／写指令／独立验证，不改代码、不推送、不打标；**打标明确只归验证存档方**；**Codex 改为自测通过后提交并推送自己那一轮**（此前口径「改完停在未提交状态」已作废）；仓库外记录目录改名为 `与统筹师的对话\`，一.5 的根目录禁区随之迁移。手册头部新增一句：权威分工见仓库外的 `团队角色条例.md`，本节与它冲突时以它为准。
- 顺带修正本文件顶部已过期的「最后追加：2026-08-22」——该行早于文件中已有的 08-28 条目，更新为 2026-08-28。
...
## 2026-08-29 本地容器四并发 LibreOffice 转换内存实测

- 在仓库外以临时 Compose override 显式改写两个具名卷，并使用独立项目启动四个 healthy 服务；按正式 RBAC 接口完成“0号→真实 developer→reviewer→两个 employee→自定义组织”的准备链，除验证码外没有库级注入账号。
- 单文件预热使嵌入模型常驻后，`zhitian-api` 空闲基线为 `677.0 MiB`（预热前冷启动 `158.1 MiB`）；两个 employee 各提交两份 Office 文档，四个请求均为 `200/accepted`，`upload_tasks` 与请求区间的实际最大重叠数均为 `4`。
- 以 `docker stats --no-stream` 循环落盘采样 `101` 次，实际平均间隔 `2.004 s`（范围 `1.401–2.049 s`），四任务期间峰值为 `783.8 MiB`，占 `2 GiB` 容器上限 `38.271%`。
- 同一已预热容器内对同规格 DOCX 做受控单次转换，LibreOffice 成功且耗时 `2093.793 ms`，较既有 `1.7–2.0 s` 记录略高；四次上传的同步受理耗时为 `1.039–1.359 s`，两组数字含义不同，不混作纯转换耗时。
- 本轮完成部署验证第3层“本地容器”资源测量；日志反证聊天端点、DeepSeek 与邮件发送均为 `0` 次。未验证检索准确性、未做审核或模型调用，本机 Docker Desktop 规格也不等同生产服务器，均不得由本轮结果推断为完成。

### 我怎么证实的（验证存档方）

- **数字自洽自己算过**：`783.8 / 2048 = 38.271484%`，与记录里的 `38.271%` 相符（`2 GiB` 取 `2048 MiB`）；`783.8 − 677.0 = 106.8`，与四并发增量相符。两项都是重算，不是照抄。
- **「soffice 被进程锁串行」这条断言看过源码才写**：`layers/converter.py:27` 是模块级 `_conversion_lock = threading.Lock()`；`:85` 的 `with _conversion_lock:` 确实包住了 `:86-91` 那次 headless `soffice` 的 `subprocess.run`；`:48` 的 docstring 也明写 process-wide lock。**由此可推出一条对读数的限定**：四并发指的是四条上传管线并发，而其中的 LibreOffice 转换是被这把锁串成一条队列的，因此 `106.8 MiB` 增量反映的是「四请求并发 + 转换串行」，不能外推为「四个 soffice 同时驻留」的开销。
- **零模型调用的边界确认被如实标注**：记录写的是「未验证检索准确性、未做审核或模型调用……均不得由本轮结果推断为完成」，没有出现「链路已验证」这类越界结论。
- **条目形态合规**：正序追加在文件末尾（hunk `@@ -1664,3 +1664,11 @@`）、标题行 + 5 条 bullet、通篇无版本号字样，符合第八章对普通工作条目的要求。
- **`.env.example` 的「零消费者」结论自己搜过一遍**，未照抄：`layers/`、`scripts/`、`tests/`、`main.py`、`config.py`、`Dockerfile`、`.github/` 各 **0 个文件命中**，仓库内 2 个 `.bat` **0 命中**，部署仓库的 `docker-compose.yml` 与其批处理同样 **0 命中**（只读核对，未修改该仓库）。全部引用只存在于 `README.md`、`docs/` 与本文件这类人读文档中。
- **四个模板值与代码默认值逐项比对**：`MAX_CONCURRENT_HEAVY_TASKS=4`、`MAX_USER_INFLIGHT_HEAVY_TASKS=2`、`MAX_CONCURRENT_INGEST_TASKS=2`、`MAX_INGEST_QUEUE_DEPTH=8`，四项与 `config.py` 的 `os.getenv` 默认值完全相同，因此模板补全后行为零变化。
- **`VERSION` 先例按要求自查后才落笔**：`git show <tag>:VERSION` 全线查得 `v3.4`/`v3.5`/`v3.6`/`v3.7` 一律停在 `3.4.0`——期间跨了 `v3.5`、`v3.6`、`v3.7` **三个两段式功能版本**，可见那是一段失修期而非「补丁号不升版本源」的约定；`v3.8` 修正后，唯一的三段式标签 `v3.8.1` 的 `VERSION` 正是 `3.8.1`。本轮据此升至 `4.3.1`。
- **未做也不该做的**：本轮未重跑任何容器测量（测试卷已 down -v 铲平，且重跑对静态复核无增益），未起 compose、未碰任何卷、未改代码或配置。`docs/claude_memory.md` 只提交、未编辑一个字。

## 2026-08-29 存档：v4.3.1 覆盖 v4.3 之后的三条非功能提交

- **本条为存档条目**。标签 `v4.3.1` 打在本条目所在的提交上，覆盖 `v4.3..v4.3.1` 共**三条**提交：`730edc8` 工作手册对齐 2026-08-28 新组织分工、`33cb842` 补全并发闸门环境变量模板、以及本轮这条（测量记录 + 验证记录 + 存档条目 + `VERSION` 同步）。三条均已在推送后打标。
- **三段式判据**：三条全为非功能改动——文档对齐、配置模板、测量记录，无任何运行时代码变化。其中 `33cb842` 是本仓库**第一次出现「只改 `.env.example`、不动任何运行时代码」的存档**，判据单列如下：该模板在代码、构建、CI 与部署编排中均无消费者（复核见上），补入的四个值又恰等于 `config.py` 现有默认值，故行为零变化；按 8.1-4「版本号必须能直接传达性质」，若打两段式会让后人误以为 `v4.3..v4.4` 之间存在功能变化。**反方论点一并记下以备复核**：模板是配置生产者面对的接口，可主张其属 8.1-3 的「部署行为」；本次未采纳，理由是那一层是人读文档而非系统行为——但这条先例值得后人重新审视。
- **`VERSION` 同步升至 `4.3.1`**。它是本仓库唯一版本文件，`/` 与 `/openapi.json` 对外报的就是它，格式不合法会让容器拒绝启动；停在 `4.3.0` 会让 `v4.3.1` 自称 `v4.3`。该文件近三轮被漏升过两次，本轮按先例复核后补上。
- **打标前门禁**：权威回归 `run_tests.bat -q` 得 **485 passed, 5 deselected**（252.2 秒）。存档前例行核对——IPv4 字面量 0 处、密钥/凭据形态 0 处、服务器绝对路径 0 处、主机名 0 处、未跟踪文件 0 个、二进制改动 0 处、跨仓库误改 0 处（另三个仓库全程零改动）。
- **本轮未覆盖什么**：未验证检索准确性；未做任何模型调用（聊天端点、DeepSeek 与邮件发送均为 0 次）；本机 Docker Desktop 规格不等同于任何服务器规格，内存读数不可外推。上述三项都不得由本轮结果推断为已完成。

## 2026-08-29 本地默认卷精排与非流式文档回答耗时测量

- `RERANK_TIMEOUT=20`下，10候选四次精排实际供应商耗时依次为`19.948 s`成功、`24.291 s`超时、`18.155 s`成功、`21.704 s`超时；5候选四次均成功，依次为`17.725 s`、`20.553 s`、`18.836 s`、`17.714 s`。观察墙钟可超过传入的20秒，以上均为单次原问题结果、没有改写重试。
- 成功精排中，10候选两条查询的hybrid前三位均有`3/3`位置变化；5候选四条分别有`2/3、2/3、3/3、2/3`位置变化。10候选成功结果的最高`rerank_score`为`10.0/9.0`；5候选四条为`9.5/6.0/9.5/10.0`，其中3条达到`8.5`。
- 四条固定查询的hybrid best score为`0.657887/0.595817/0.594249/0.634849`，仅`1/4`达到`0.65`。fast证据筛选分别选中`3/0/2/5`条；在10候选精排成功的两条查询中，所选证据与精排前三名合计重合`4/5`，并覆盖精排前三名的`4/6`。
- `RERANK_ENABLED=false`且回答预算40秒时，三条全新动态问题的非流式文档回答分别为：长综述`44.697 s`后超时、返回800字符保守摘要；具体条款型`18.677 s`成功、生成284字符、距40秒余`21.323 s`；跨文档比较`41.689 s`后超时、返回800字符保守摘要。最长一条相对40秒的实测余量为`-4.697 s`；按同一墙钟反事实判断，25秒只够中间一条，本轮没有为此追加第二轮付费调用。
- 测量使用默认卷但保持常驻API停止，以相反环境变量启动相互独立的一次性容器；前后均为5个账号、6份文档（5 verified、1 pending）与382条文档向量，临时脚本和容器已清理。本轮未验证精排对最终回答质量的影响，本机网络延迟不等于生产延迟，未触碰生产配置，也没有经过Office转换路径。

## 2026-08-29 本地默认卷fast档精排耗时对照测量

- 第0步复核确认上一轮A部分向`memory.search_documents()`传入的是`tier="expert"`；本轮保持`RERANK_ENABLED=true`、`RERANK_TIMEOUT=20`、10候选与四条查询不变，仅改为`tier="fast"`，共执行4次付费精排调用且未重试。
- fast档四条实际供应商耗时依次为`8.832 s / 3.741 s / 8.284 s / 5.701 s`，全部成功；对应pro档为`19.948 s成功 / 24.291 s超时 / 18.155 s成功 / 21.704 s超时`。fast档未启用强制JSON响应格式，但四条均被`_parse_rerank_scores()`正常解析，JSON解析失败为`0/4`。
- A查询的fast与pro前三名按`(doc_id, chunk_index)`完全一致；C查询前两名一致，第三名由pro的网络安全PPTX chunk 10变为fast的法律基础文本 chunk 41，因此两条可比查询中`1/2`前三完全一致。B、D的pro档均超时，只有fast档产生了可比较的精排结果。
- 测量保持默认API容器停止，以挂载只读测量脚本的一次性容器直接调用检索层；前后均为5个账号、6份文档（5 verified、1 pending）、382条文档向量与5条长期记忆，一次性容器和临时脚本已清理。本轮只记录模型档位对耗时与排序的影响数据，不据此判断精排是否应启用，也未验证最终回答质量或生产环境延迟。

## 2026-08-29 修正Expert文档精排档位、预算与失败隔离

- 精排固定使用`deepseek-v4-flash`快档并始终请求JSON响应格式，`RERANK_TIMEOUT`代码默认值由`5.0`暂调为`12.0`；依据是本机fast档四条真实测量最慢`8.83 s`，代码注释明确该值仍待生产网络实测复核，未改fast路径的`rerank_enabled=False`或候选池。
- 精排超时、限流或连接失败仍记录`document_rerank_timeout/deepseek_rate_limit/deepseek_upstream_unavailable`等结构化原因，但不再打开请求级DeepSeek熔断；hybrid顺序兜底后继续文档回答，搜索词改写、反思、重规划与输出观察不再被这项可选增强的失败连带跳过。
- 默认卷真实验证中，A/C两次精排均确认实际调用`deepseek-v4-flash`、携带`response_format={"type":"json_object"}`与`timeout=12.0`并成功，供应商墙钟分别为`10.413 s/12.817 s`；后者略超参数但成功，再次证明供应商超时不是严格墙钟。异常注入验证精排失败后原因码仍精确保留、熔断保持关闭且文档回答继续执行。
- A/C各自基于hybrid前五与精排前五生成四段对照回答：A-hybrid `32.168 s`成功、A-rerank `41.571 s`超时后返回保守摘要，C-hybrid `41.578 s`超时后返回保守摘要、C-rerank `35.128 s`成功；该原文仅供用户判断，本条不评价精排对回答质量的影响。

## 2026-08-29 文档回答改为流式生成并移除原文倒灌兜底

- Expert文档回答复用既有SSE流式模式：检索与citations先完成，正文逐chunk交付，citations仍在正文之后作为独立事件发送；公开的非流式`/chat`继续收集同一生成器并返回字符串，接口契约不变。供应商timeout现约束连接/单次网络读取等待，expert的`complex_deadline`在chunk间继续检查；供应商阻塞读取仍可能越过名义墙钟，因此不把timeout描述为精确计时器。
- 删除两处`conservative_summary[:800]`原文倒灌：首块前失败时只返回“已取得知识库文档依据，但模型整理失败/超时”的明确说明；已有输出后失败则保留已生成部分并在末尾追加说明。两种情形均保留citations，测试同时断言PDF/PPT/OCR原始噪声不会作为回答泄出。
- 三条原问题真实流式复测：长综述由改造前`44.697 s`超时变为首块`75.144 s`、完整`91.362 s`成功（1686字符）；具体条款由`18.677 s`变为首块`7.465 s`、完整`9.220 s`成功（223字符）；跨文档比较由`41.689 s`超时变为`129.333 s`在首块前超时并返回25字符明确说明。三次均未倒灌原文，后者说明40秒供应商read timeout仍非可见首字节的严格墙钟。
- `.env.example`的`RERANK_TIMEOUT`由已证实失败的`5.0`同步为代码默认`12.0`，明确标注为本机实测暂定值、待生产复核。本轮未判定精排对最终答案质量的影响，本机耗时不等于生产耗时，知识库未命中时的模型自答与生产配置均未处理，不得据此视为已完成。
- Python语法检查通过，精排/降级针对性`27 passed`，权威入口`run_tests.bat -q`为`485 passed, 5 deselected, 0 failed in 390.77s`。本轮未做生产实测、未改非流式文档回答结构，也未据本机结果判定精排质量；上述边界不得视为已完成。

## 2026-08-29 为Expert文档回答增加独立首正文墙钟上界

- 新增环境变量`FIRST_CONTENT_TIMEOUT=45.0`，实际等待取该值与请求剩余总预算的较小者；45秒由Cloudflare首字节100秒、生产请求总预算90秒及此前首块后约16秒正文输出耗时倒推，代码与模板均明确标为暂定、待生产实测复核。
- 首正文等待改由继承当前请求凭据上下文的守护线程执行，调用线程独立等待`Future`并在到期时主动关闭供应商流；因此即使连接持续产生reasoning等非正文事件，墙钟上界仍会触发，且个人DeepSeek Key不会在线程切换时误退回企业Key。
- 首正文超时使用独立原因码`document_first_content_timeout`，返回明确的模型整理超时说明，不倒灌检索原文；SSE实测与自动化断言均确认失败正文之后仍发送独立citations与降级状态事件。expert复测中长综述由首块`75.144 s`/完整`91.362 s`改为`45.003 s`明确失败，跨文档比较由`129.333 s`改为`45.002 s`明确失败；具体条款题本次遭遇供应商`ReadError`，在`4.279 s`安全失败，未重跑也未把它记作正常完成。
- 保持默认档位不变的独立fast测量中，三题首正文分别为`15.812 s / 3.440 s / 6.897 s`，完整输出为`20.476 s / 3.597 s / 11.016 s`，均成功且无降级原因；本轮只提供原文与耗时供用户判断，不据此切换Expert回答档位。
- Python语法检查、文档回答/SSE/系统模块/降级针对性`82 passed`，权威入口`run_tests.bat -q`为`493 passed, 5 deselected, 0 failed in 250.11s`。fast档是否采用未决，45秒仍是暂定值，本机延迟不等于生产延迟，三项均不得由本轮结果推断为已完成。

## 2026-08-29 按Expert执行阶段集中分配模型档位

- `config.py`新增完整、可审计的Expert阶段→模型档位映射与统一取值入口；fast请求无论阶段映射为何均强制保持fast，复杂任务分解、checkpoint、ReAct、直接推理、意图分类、复杂汇总与输出观察继续使用expert。
- 文档精排沿用fast不变，并将文档回答、流式/非流式搜索整理、附件上下文回答、历史上下文润色、搜索词改写与长期记忆重要性判断统一改用fast；请求模式与实际模型档位分离，Expert专用提示词及输出观察安全控制没有随整理阶段降档。
- 同一条联网问题、同一组5条Tavily结果的首次真实流式对照中，pro档首正文`80.909 s`、完整`107.479 s`，fast档首正文`26.554 s`、完整`39.263 s`；中文原文因首次终端编码错误另补一次仅用于取回原文，未拿第二次缓存可能影响的耗时替换首次数据。
- Python语法检查通过，权威入口`run_tests.bat -q`为`498 passed, 5 deselected, 0 failed in 359.38s`。意图分类与复杂任务汇总仍待独立测量，输出观察因属安全控制未降档，本机网络耗时不等于生产延迟，均不得由本轮结果推断为完成或定论。

## 2026-08-30 补齐流式搜索首正文上界并隔离输出观察失败

- 将文档回答已有的首正文守护器抽成通用LLM流机制，流式搜索整理复用同一`FIRST_CONTENT_TIMEOUT`与请求剩余预算钳制；守护线程会在供应商持续产生非正文活动时独立按墙钟中止，失败只返回“已取得联网搜索结果，但模型整理超时”的明确说明，不倒灌搜索原文。
- 流式搜索在首正文后也按chunk检查Expert请求总预算；输出观察校验失败则先按timeout/rate_limit/upstream_unavailable分类并记录原因码，不再打开请求级DeepSeek熔断，既有模型档位与`OUTPUT_ANOMALY_CHECK_TIMEOUT=5.0`均未修改。
- 唯一一次真实Expert联网测量中，搜索词改写在`9.956 s`后超时并先行打开熔断；Tavily检索`2.345 s`，fast整理首正文`6.209 s`、完整`8.242 s`，总耗时`20.548 s`且产出295字符。输出观察函数被调用但因熔断守卫未发起模型请求，因此本轮没有得到其真实耗时或5秒预算充分性数据，并按“一次提问、不重跑”边界停止。
- Python语法检查与搜索/降级针对性`41 passed`，权威入口`run_tests.bat -q`为`501 passed, 5 deselected, 0 failed in 396.20s`；新增测试实证无正文活动仍触发首正文上界、上界受请求预算钳制、失败不泄露原始搜索结果，以及输出观察超时保留原因码但熔断保持关闭。
- 本轮未改变任何模型档位或既有预算值；输出观察的Expert档位与5秒预算仍待一次真正执行后的数据复核，尚无生产验证，本机耗时不等于生产耗时，以上均不得由本轮结果推断为完成或定论。

## 2026-08-30 修正搜索词改写预算与失败隔离

- `SEARCH_QUERY_REWRITE_TIMEOUT`由`4.0`暂调为`15.0`并同步`.env.example`；依据为本机fast档真实改写`9.956 s`与供应商timeout可能比设定墙钟多约5秒，代码和模板均明确标注待生产网络实测复核，实际调用仍受请求剩余预算钳制。
- 搜索词改写的timeout/rate_limit/upstream_unavailable仍分别记录`query_rewrite_timeout/deepseek_rate_limit/deepseek_upstream_unavailable`并回退原始query，但不再打开请求级DeepSeek熔断；后续联网整理与输出观察可继续执行，预算耗尽提前返回分支与异常分支行为由此一致。
...
## 2026-09-02 暴露Uvicorn可信代理配置并固化未认证限流分桶边界

- 新增标准环境变量`FORWARDED_ALLOW_IPS`，由`config.py`、`python main.py`直启入口与Dockerfile容器入口共同消费；默认值保持Uvicorn既有的`127.0.0.1`，没有默认信任`*`或硬编码任何部署代理地址。
...
- **`API_DOCS_ENABLED` 的解析规则实测而非读字面量**：复刻 `config.py:131` 的表达式跑了 15 组输入。**只有归一化后恰为 `true` 的输入会开启**——`'true'`/`'True'`/`'TRUE'`/`' true '` 四种开启，`'ON'`/`'on'`/`'1'`/`'yes'`/`'ture'`/`''`/`'  '`/`'trueX'`/`'0'` 与未设置全部关闭。倒向安全成立：认不出来的一律关。
  ⚠️ 但要精确说明一点：`True` 与尾随空格**并不是"落到关闭"，而是被 `.lower()`/`.strip()` 归一化后照常开启**。把它们当成"写错了也会关着"是危险的理解——在生产 `.env` 里写 `API_DOCS_ENABLED=True` 会真的把三个文档端点打开。
- **三个端点全部受控，无遗漏**：`main.py:221-223` 的 `docs_url`/`redoc_url`/`openapi_url` 各自由同一开关决定，FastAPI 的文档端点也仅此三个。`.env.example:72` 为 `false`。
- **那处测试改动是「契约翻转」而不是「放宽断言」**，判据是**检测力的方向而非强弱**：放宽意味着新旧两种行为都能通过（例如删掉断言或改成 `in (200, 404)`），检测力下降；这里断言由 `== 200` 改为 `== 404`，一旦有人把文档端点改回默认公开，该用例**会失败**——检测力被保留并反转指向。用例同时改名为 `..._and_default_openapi_is_hidden`，把新契约写进了名字而非隐含在断言里。`GET /` 的版本断言仍在（`tests/test_observability.py:215-216`）。
- **开关两态均有覆盖**（此处更正我自己的一个中途误判）：我最初只搜了 `test_observability.py` 便怀疑"开启分支无测试"，扩大到 `tests/` 全目录后发现本批新增了 `tests/test_api_documentation.py`，两条用例分别断言关闭时三端点全 `404` 且版本为 `None`、开启时全 `200` 且版本等于 `VERSION`；且它用独立子进程重建运行时——因为 `docs_url` 是模块级构造，不重新导入就测不出开关效果。**"没有"这个结论必须先说清搜索范围**，我第一次没做到。
- **`FORWARDED_ALLOW_IPS` 的空值兜底两条路径都实测过**：`config.py:125-127` 的 `.strip() or "127.0.0.1"` 使未设置／空串／纯空格**全部落到 `127.0.0.1`**，绝不产生 `*`；Dockerfile 的 `${FORWARDED_ALLOW_IPS:-127.0.0.1}` 对未设置与空串同样落到回环。
  两条路径有一处**非对称**（不构成安全问题，但记下）：纯空格输入时 `config.py` 归一化为回环，而 shell 的 `:-` 不触发、会把 `"   "` 原样传给 uvicorn；其失败方向是"谁都不信任、退回直连对端 IP"，倒向安全而非倒向 `*`。
- **两条启动路径都接入**：`main.py:3869` 传 `config.FORWARDED_ALLOW_IPS`，`Dockerfile:93` 的 CMD 传 `--forwarded-allow-ips`。
- **三条测试断的是分桶键实际取值，不是"参数已传入"**：前两条经**真实的 `uvicorn.middleware.proxy_headers.ProxyHeadersMiddleware`**（非 mock）处理 ASGI scope 后读 `main._rate_limit_key()`——可信代理链得 `anonymous:198.51.100.23`，不可信对端伪造同一 XFF 时仍得 `anonymous:203.0.113.50`，即伪造不能自选桶；第三条断言容器入口文本与模板，并含 `FORWARDED_ALLOW_IPS=*` **不存在**的反向断言。
- **代码与模板中无任何具体代理地址**：`config.py`/`Dockerfile`/`.env.example`/`main.py` 对本批三个 IPv4 零命中。三个字面量**只存在于 `tests/test_forwarded_rate_limit.py`**，其中 `198.51.100.23`、`203.0.113.50` 属 RFC 5737 保留的文档示例网段，`172.18.0.5` 属 docker 默认 bridge 私有网段，均为测试夹具而非真实部署地址。
- **未做的**：未重跑任何测量、未起 compose、未碰卷、未改代码或配置；`docs/claude_memory.md` 只提交、未编辑一字。

## 2026-09-02 存档：v4.5 覆盖 v4.4 之后的三条提交

- **本条为存档条目**。标签 `v4.5` 打在本条目所在的提交上，覆盖 `v4.4..v4.5` 共**三条**：`3ced6ae` 为未认证限流分桶新增可信代理配置入口、`07e3ac9` 默认关闭FastAPI接口文档端点、以及本轮这条。
- **两段式依据**：`07e3ac9` **改变了默认行为**——`/docs`、`/redoc`、`/openapi.json` 三个端点由公开变为默认关闭，属实质行为变化而非纯配置能力新增，按 8.1 第 3 条不能用文档补丁号掩盖。`3ced6ae` 单独看默认行为未变（默认仍是 Uvicorn 原有的 `127.0.0.1`），但同一存档中只要含一项实质变化即整体按两段式处理。
- **`VERSION` 同步升至 `4.5.0`**。它是本仓库唯一版本文件，`/` 与 `/openapi.json` 对外报的就是它，格式不合法会让容器拒绝启动。该文件近几轮被漏升过两次，本轮按要求补上。
- **打标前门禁**：权威回归以 `run_tests.bat -q`（项目 `.venv`）实跑得 **506 passed, 5 deselected**（250.4 秒），与指挥师 2026-09-02 的数字一致。存档前例行核对——密钥/凭据形态 0 处、服务器绝对路径 0 处、主机名 0 处、未跟踪文件 0 个、二进制改动 0 处、跨仓库误改 0 处；IPv4 字面量 3 个，来源与性质见上，均为测试夹具。
- **本轮未覆盖什么**：①**两项配置的实际修复都要在部署时设置才生效**，代码侧只做到「可被正确配置」——`FORWARDED_ALLOW_IPS` 不设则未认证限流继续全局共用一个桶，`RERANK_ENABLED` 仍是 v4.4 遗留的待处理项；②`GET /` 的版本号按裁定保留，未随文档端点一并隐藏；③**nginx 层未做任何遮挡**，本轮只在应用层关闭文档端点；④生产是否已生效不属本轮范围。

## 2026-09-04 校准部署指南的VPC绑定与TLS现状

- 更正`SERVER_PUBLIC_IP`的宿主机绑定说明：Docker发布端口应使用当前网络拓扑下宿主机实际可绑定、能够承接入口流量的地址；公网地址经VPC网关DNAT时不再错误要求该公网地址必须直接存在于宿主机网卡。
- 将真实域名、DNS、HTTPS证书、80到443跳转及证书续期从Phase B待办更正为已完成；本轮没有修改任何Nginx指令、端口、环境变量或运行时代码，也未接触生产环境。
- 新增到同级`zhitian-deploy`仓库`README.md`与`.env.example`的证书配置指路，不在本指南复制uid 101权限规则，避免同一要求多处维护后漂移。
- 本轮为纯文档校准，使用差异检查、相对路径核对与敏感值扫描验收；没有运行权威回归，也不据此声称生产配置发生变化。

## 2026-09-04 将部署指南从服务器快照改为可重复验收流程

- 找全并移除服务商、地域、带日期运行状态、现场Docker/Compose版本、固定镜像体积及旧标签组合等易漂移快照；历史事实继续由`CHANGELOG.md`保存，实例状态改由部署单现场记录。
...
## 2026-09-04 修复三库全缺失时无法执行灾难恢复

- `restore_data.py`在恢复前自动判定三个SQLite主库状态：三库齐全时继续生成独立前缀的加密恢复前安全备份；三库全缺失时无需操作者记忆额外参数，自动跳过预备份并进入既有的归档校验与事务切换流程。
...
## 2026-09-05 新增共用调度判据的运维备份状态端点

- 新增缺省关闭的`GET /ops/backup-status`：仅配置`OPS_STATUS_TOKEN`时注册路由，请求头使用`hmac.compare_digest`校验；未配置为`404`、错误令牌为`401`、正确令牌为`200`，`unknown`状态也保持`200`供聚合层区分服务不可达与状态不可判定。
- 将UTC+8调度窗口、最新调度归档`st_mtime`及宽限期比较抽成`describe_scheduled_backup_state()`，调度器与端点共用同一判据；只匹配`zhitian-scheduled-backup-*.ztbackup`，手工备份与恢复前快照不参与判断，响应不暴露文件名、路径或归档数量。
...
## 2026-09-24 纯修 x.y.Z（v4.8.1）：为已知漏洞建立限时、按包例外并封闭扫描跳过漏洞，用户操作方式不变

- 以容器 CI run `35955469586` 的原始 JSON 为基线，建立单一 `.trivyignore.yaml`：32 个不重复 CVE 逐条记录受影响包、可达性证据、未修理由与 30/90 天到期日；Chroma 三项同时保存 CVE 与 pip-audit 的 PYSEC 对应关系。
- pip-audit 从同一文件取三项 `--ignore-vuln` 编号，Trivy 门禁从同一文件按 PURL 过滤；完整 Trivy 报告仍保留未过滤数据。到期/字段缺失时预检失败，三个扫描步骤只要不是 `success`（含 `skipped`、`cancelled`）即在最终策略步骤报出具体步骤与结果。
- 阶段 C 的 LibreOffice 进程树禁网是 libcurl 四项判为不可达的前提；libxml2 的上传 XML 解析路径按可达管理，系统 expat 的具体解析分支证据不足按未能判定管理，上传图片解码的 libtiff 风险按可达管理。
- 本机对下载的原始 Trivy 报告做按包过滤和删除 Chroma 例外的反向验证，移除 `CVE-2026-45830` 后门禁重新报 HIGH；pip-audit 移除对应 `PYSEC-2026-3813` 后也重新报告该漏洞，过期与跳过扫描的反证由测试覆盖。生产镜像与后续新增漏洞仍须由推送后的 CI 复核，本轮未推送、未打标。

## 2026-09-24 存档：v4.8.1 —— 纯修：门禁恢复可信、日志不再被吞、文档转换禁止外连、pypdf 安全升级（存疑：转换结果不再显示文档内的远程图片）

- `7b30502`：更新 8.1 版本号定档规则。
- `294e32d`：同步指挥师文档中的 v4.8 状态、版本号新规则与遗留事项。
- `7b3a066`：自动登记项目日志，并让定时备份完成日志以 INFO 级别在控制台可见。
- `69d3ed6`：将 pypdf 升至 6.19.0，`VERSION` 升至 `4.8.1`。
- `47bae9f`：为 LibreOffice 转换进程树添加禁网隔离。
- `228fb86`：避免构建期禁网探针污染 `/app/data`，校准转换排障说明。
- `8125425`：建立限时、按包的漏洞例外清单，并使扫描跳过或取消时门禁失败。
- 以上为 `git log v4.8..HEAD` 在本存档提交前核得的 7 个提交；本条只整理存档与协作文档，不代表已部署或已完成生产验证。

## 2026-09-24 修复 F39：关闭 Chroma 时释放底层系统与共享缓存

- `memory.close_resources()` 沿用仓库备份与迁移脚本的顺序：先停止 Chroma 的 `_system`，再清理 `SharedSystemClient` 缓存并丢弃模块引用；Chroma 0.5.0 没有公开的客户端 `close()`。
- 新增回归测试：关闭后旧系统已停止且缓存无该客户端，随后的文档检索会重新初始化并命中原切片。此改动不迁移或重写已有向量数据；未在生产进程上验证。

## 2026-09-24 修复 F48：文档入库按真实批次上报进度并校验最终切片数

- 新增 `INGEST_CHUNK_BATCH_SIZE=32`（可配置，32 片兼顾中型文档进度可见性与最多 2000 片时的提交次数）；每批 Chroma 写入成功后才更新 `processed_chunks` 与真实比例，末批经 `doc_id` 实际条数核对后才写 `done/100%`。
- 保持原有切片、随机 ID 与 metadata 形态；处理中提前登记 `result_doc_id` 以供重启清理，失败重试前复用 `_purge_partial_document()` 按 `doc_id` 删除所有已写批次并归零进度；SSE 字段未变。
- 回归测试覆盖中间值、最终条数不符、第二批失败及存量 metadata 形态。本机经真实 DOCX 上传得到 40 片，记录到的进度为 `0/40→8/40→16/40→24/40→32/40→40/40`（0→20→40→60→80→100%）；未在生产数据上验证。

## 2026-09-24 补齐登录审计且不改变认证接口契约

- 登录成功与失败各记录一条 `[audit]` INFO：成功记录 `user_id`，失败记录用户名 HMAC-SHA256 前 16 位；使用启动必需的企业密码种子并加独立域前缀，不复用备份或个人 Key 加密密钥。
- 记录请求角色、由可信代理链解析后的 `request.client.host` 与失败类别（凭据错误、账号禁用、配置错误、其他异常）；角色和地址先转义，不记录明文用户名、密码、邮箱或令牌。
- 复用专用 INFO 控制台通道让审计事件进入 `docker logs`，全局控制台仍保持 WARNING；本机测试核对单行、摘要稳定性、伪造转发头不生效及原有错误响应逐字不变。生产访客 IP 的正确性仍取决于部署时可信代理配置，尚未在线验证。

## 2026-09-24 修复 F14/F22：模型请求复用共享连接池并归还放弃的流

- DeepSeek 调用改为每次按请求 Key 创建短生命周期 SDK 客户端，但共用一个 HTTP 连接池；连接上限 16、空闲保留上限 8，保守 1 秒过期；共享池拒收及拒发 Cookie，不按 Key 缓存客户端。
- 首正文超时、预算耗尽、异常、SSE 客户端断开及未迭代即放弃均显式关闭模型流；只有可证明发生于发送前的连接重置才额外立即重试一次，不改既有熔断分类和超时重试次数。
- 回环服务测试证实连续 5 次只建 1 条连接、不同 Key 不串、Set-Cookie 不跨请求，2 条连接上限下连续放弃 5 个流后第 6 次仍在 1 秒内取到连接；权威回归 544 passed、5 deselected。
- 本机同一短输入各做 10 次 fast 调用：改动前耗时中位数 2285.5 ms，改动后 1233 ms；首调用分别为 2373/3733 ms，后 9 次中位数分别为 2198/1225 ms。两组输出均因 `max_tokens=8` 无正文，仅用于传输耗时比较；网络波动与供应商负载未控制，不能视为生产性能承诺。

## 2026-09-25 调查 GraphRAG 的启用、存量文档与删除路径

- 新增一行 `[graphrag]` 启动 INFO 日志：关闭时只记录 `disabled`、不查表不建表；开启时逐一报告三张图谱表是否存在。使用第 0 批的专用控制台通道，新增启动测试验证关闭时表不会出现。
- 本机 Windows `.venv` 使用仓库外独立临时数据目录与真实 ONNX 嵌入模型：关闭时入库 4 片，三张图谱表均不存在；开启后入库另一份 3 片 Markdown，抽取每片耗时 39.804/17.296/17.784 秒，建得 91 个实体、86 条关系、94 条切片关联。
- 四个问题的检索第 1 名在开关两态下均未变化，图扩展额外加入候选分别为 0/2/0/0；先前在关闭状态入库的文档仍可混合检索，但图谱关联为 0，开关打开不会补建。按本机暖态每片约 17.5 秒粗估，258 片至少 258 次抽取调用、串行耗时约 76 分钟，不含失败重试。
- 删除唯一有图谱的文档后，其 3 个 Chroma 切片与 94 条切片关联已清，但 91 个实体及 86 条关系仍留在 SQLite；这是本轮发现但未修的清理边界，不能称为图谱彻底清除。
- 默认 `data/` 前后 16 个文件的相对路径、大小与 UTC 修改时间逐项一致，临时目录已清理；权威回归 546 passed、5 deselected。Docker 在本次执行环境不可用，本机原生结果不等于生产 Linux 容器验证，本轮不决定 GraphRAG 修复或下线。
