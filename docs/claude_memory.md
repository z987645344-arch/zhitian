...
> **最后更新：2026-08-22**（用户自选API额度来源的后端与customer网页端已实现并完成权威回归，等待指挥师打标前审查）
...
| 项 | 说明 |
|------|------|
| 状态 | 🟢 自用云端MVP Phase A功能验证已闭合，Phase B服务器落地进行中；2026-08-22完成“用户自选API额度来源”的后端与customer网页端实现，权威回归`428 passed, 5 deselected`。当前没有已知开放P0/P1，但尚不具备不可篡改安全审计，也不等于所有接口都完成形式化安全证明。开放问题仍为L9、F14、F21、F22、F24、F38、F39、F44、F48；后端容器漏洞策略门禁因F38及Debian系统层无修复版本项保持红灯，不代表应用功能回归 |
| 上一轮完成 | 2026-08-22完成用户自选API额度来源：个人DeepSeek Key采用独立密钥的AES-256-GCM密文落库；企业流动密码一次授权永久有效，失败5次按账号锁12小时；同步聊天、SSE线程与长期记忆任务使用同一显式来源且不自动回退；customer网页新增暗色设置页。接口/日志/错误信息/备份四处不外泄专项通过，桌面与390px布局通过。本轮仅改`zhitian`后端与`web_client`，未触碰Flutter、管理后台或部署仓库 |
| 当前等待 | 等待指挥师按打标前清单审查本轮功能；版本号与标签尚未决定。Flutter客户端与管理后台若要接入同一设置能力，必须分别另开独立技术栈批次，本轮没有留下跨仓库半成品。服务器部署前必须新增`PERSONAL_DEEPSEEK_KEY_ENCRYPTION_KEY`，否则新版按安全设计拒绝启动；既有用户迁移后额度来源为空，需本人首次登录设置，不做隐式企业额度兼容回退 |
