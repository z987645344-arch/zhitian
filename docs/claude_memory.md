...
| Git 存档 | ✅ 三项目 v1.0/v2.0 均已 commit；v2.1：后端`8dc9730`、管理后台`f9603eb`、客户端`8886e42`，已推送并打tag v2.1。<br>**2026-07-28 最新存档（tag v2.5）**：后端`1e30790`（F26/F27两项P0越权修复、F28按doc_id精确删除、F29组织删除前置检查、F30测试统一隔离、按组织文档统计、requirements 26项全部锁定，权威回归`338 passed, 5 deselected`）、管理后台`60ef88e`（组织统计chip展示+删除改传doc_id，`node --check` 9文件通过），两仓库commit与v2.5标签均已推送。**客户端`zhitian_app`本批无任何改动，工作区clean，标签保持`v2.2`未新增**（不为版本号整齐而强行打标签）。<br>此前：v2.4后端`0428ff3`（GraphRAG，仅后端）。<br>**2026-07-26（tag v2.3）**：后端`4dbc0d8`（README刷新，v2.3标签指向此提交；上一条`053fa67`为组织加入退出审批体系 + 文档组织归属机制 + 移除前端/后端模板文件夹 + GraphRAG评估记录，权威回归`317 passed, 5 deselected`）、管理后台`f014267`（组织大厅、审批队列与上传组织选择器，`node --check` 9个文件通过，v2.3标签指向此提交），两仓库commit与v2.3标签均已推送至origin。**客户端`zhitian_app`本批无改动**，仍停留在`9b2f904`，标签保持`v2.2`未新增。本地`.env`及运行产物持续被`.gitignore`排除未推送 |
