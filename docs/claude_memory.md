...
| ⚠️ `zhitian_admin` `v3.2.1`标签注释内含两处已固化的不准确 | 标签已推送、按红线不删不重打，因此**引用该注释时必须知道**：①注释称`b7e0a8e`为「38增11删」，实际是**27增11删**——38是`git show --stat`那条`README.md \| 38 +++---`的**改动行总数**，不是插入数，插入/删除只能读`--numstat`或`--shortstat`；②注释称基线扫描「0漏洞」，而该门禁步骤的`severity`只取`HIGH,CRITICAL`，故它证的是**0个HIGH/CRITICAL**、非全严重度零漏洞。另注释中「带3.5.7-r0的镜像此刻就在运行」是生产状态断言，与同一注释末尾「线上版本我这边未见」自相矛盾——该句源自指挥师给出的文案，教训是**永久注释里不得写只有统筹师能证实的生产状态** |
