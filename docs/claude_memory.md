...
> **最后更新：2026-08-28**（Expert请求级熔断已覆盖超时、429限流、连接失败与HTTP 5xx；业务错误保持不熔断）
...
| 项 | 说明 |
|------|------|
| 状态 | 🟢 自用云端MVP Phase A功能验证已闭合，Phase B服务器落地进行中；后端当前最新标签为`v4.2`，Expert执行动态及请求级上游熔断位于该标签后的`master`提交/工作区，尚未打标。最新权威回归`485 passed, 5 deselected, 0 failed`；本轮尚未提交推送，因此没有可归属于本轮的新CI运行，不能用此前CI结果冒充。当前没有已知开放P0/P1，但尚不具备不可篡改安全审计，也不等于所有接口都完成形式化安全证明。开放问题为L9、F14、F22、F24、F39、F48 |
| 上一轮完成 | 2026-08-28把Expert请求内DeepSeek熔断从“只认超时”扩展为“上游暂时不可用”：timeout、429 rate limit、连接失败与HTTP 5xx均会停止后续可选模型阶段并只保留一次最终回答机会；400/401/403/422、参数错误和内容审核拒绝等业务错误仍归`other`且不误熔断。新增`deepseek_rate_limit`与`deepseek_upstream_unavailable`结构化原因及网页中文提示，供应商错误计数同步增加新类别；超时数值与请求级作用域均未改动 |
| 当前等待 | 本轮不部署；版本号与标签仍由指挥师决定。用户已明确打标不必等待Flutter执行动态补齐，Flutter继续作为独立后续任务，不再作为后端当前版本的打标阻塞项 |
...
| Git 存档 | ✅ 后端最近已审稳定标签为`zhitian v4.2`；Expert执行动态已进入其后的`master`提交，请求级上游熔断扩展仍在工作区，均尚未打标，不得冒充已经进入`v4.2`。用户已决定后端打标不必等待Flutter执行动态。部署仓库、管理后台、Flutter客户端仍按各自独立标签演进，本轮明确未修改。后端`VERSION`仍是应用版本源，标签与部署检出必须显式指定，不跟随默认分支；漏洞策略仍因Chroma服务端不可达记录及系统层B/C类HIGH/CRITICAL保持红灯 |
