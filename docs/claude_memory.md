...
> **最后更新：2026-08-16**（部署仓库完成双子域名与源站HTTPS的仓库侧改动，服务器侧签证书/DNS/线上验证待办）
...
| 项 | 说明 |
|------|------|
| 状态 | 🟢 自用云端MVP Phase A功能验证已闭合，Phase B服务器落地进行中；当前没有已知开放P0/P1，但尚不具备不可篡改安全审计，也不等于所有接口都完成形式化安全证明。开放问题为L9、F14、F21、F22、F24、F38、F39、F44、F48；后端容器漏洞策略门禁因F38及Debian系统层无修复版本项保持红灯，不代表应用功能回归 |
| 上一轮完成 | 2026-08-16 `zhitian-deploy`分两轮完成双子域名与源站HTTPS的**仓库侧**改动，均未提交：①Nginx改为官方镜像模板机制，拆成「8080跳转+健康检查」与两个按`server_name`分流的8443块，Compose新增443映射、证书只读挂载与`ZHITIAN_*`变量；②新增`ZHITIAN_FORCE_HTTPS`开关恢复本机开发通道（`off`时8080保留完整旧HTTP路由，`on`时行为与①完全一致，`/api/ready`两种模式都直通），新增`生成本机自签证书.bat`与被忽略的`local-tls/`，并修正①中「宿主机路径＝容器内路径」的证书挂载写法——Windows下不可用，已改为固定挂到`/etc/nginx/tls/`。本机已用真实四服务跑通两种模式的完整验收；证书签发、DNS与线上验证仍在服务器侧。**③指挥师本人在验收后改动三处**（经用户逐次授权的一次性例外）：8080守卫由`= on`改为`!= off`，使`ON`、末尾空格等笔误倒向生产行为而非落到else分支——按原写法一个大小写错误即可把企业管理后台重新暴露在公网IP的明文HTTP根路径上；同步改写模板注释与`.env.example`说明。已实测`on`/`off`/`ON`/`typo`四种取值`nginx -t`全部通过、守卫5处、`/api/ready`仍无守卫、nginx自身变量全部幸存 |
| 当前等待 | **生产实例已于2026-08-13迁至染云数据香港（Ubuntu 24.04）；此前的腾讯云实例因跨境网络质量问题已退款作废，任何文档、脚本或操作都不应再指向它。** 四服务全部healthy；同机多IP入口与Compose `env_file.format: raw`加固统一归入后端/部署仓库`v3.3`。2026-08-16已通过SSH完成两项前置核对并**全部通过**：Compose实际版本`v5.4.0`（`format: raw`要求≥2.30.0）、后端`.env`共18个`KEY=value`且**零个值带引号、零个值含`$`**（核对只读取变量名，未读取任何值）。服务器当前检出后端与部署仓库`v3.3`、管理后台`v3.2`。**`zhitian-deploy`工作区现有7项未提交改动**（`nginx/compose-nginx.conf`→`.conf.template`、`docker-compose.yml`、`.env.example`、`.gitignore`、`README.md`、`CHANGELOG.md`，以及新增未跟踪的`生成本机自签证书.bat`），等待用户确认提交；随后由指挥师在服务器侧签发Origin CA证书、添加两条DNS记录、现场填写`.env`（四项`CHANGE_ME_*`真实值 + `ZHITIAN_FORCE_HTTPS=on`；私钥须`chown root:101`+`chmod 0640`，否则以uid 101运行的反代会因无权读取私钥启动失败）、重建反代容器并完成线上验证，最后再升级`v3.4`。不得把本机验证冒充生产结果。网页版批次二剩余文件库、工具箱、欢迎页/附件展示等体验项根据后续指令安排；F44与F48继续保持P3待讨论 |
...
| 下一步 | 当前已知待办汇总：①**Phase B正式域名接入——已确定使用`agent.zhiliaohub.com`**（与同机的知了Hub共用同一顶层域名，按子域名分流到本项目专属IP；域名解析与跳转属跨项目协调背景，不构成运行依赖，真实值仍只进未跟踪`.env`）。**入口已按双子域名拆分**：客户端与管理后台各占一个主机名、互不同源，避免客户端XSS读走管理后台令牌，也避免访问根域直接看到企业后台。**仓库侧443监听已于2026-08-16在`zhitian-deploy`完成（未提交），本机开发通道同日经`ZHITIAN_FORCE_HTTPS=off`恢复并实测通过**，剩下的阻塞项全部在服务器：该域名DNS权威在Cloudflare且SSL/TLS模式为zone级，直接开代理会以HTTPS回源失败返回502，因此必须先签**Cloudflare Origin CA证书**（免费、覆盖根域与一级通配）并放到`.env`指定路径——**不要用certbot**，共享服务器上`certbot --standalone`默认绑`0.0.0.0`会与知了Hub冲突。另需在接入前实测**Cloudflare免费版100秒响应上限与expert 120秒全局预算的冲突**（预期真流式`/chat/stream`因持续有数据与心跳不受影响，非流式`/chat`超100秒会被`524`掐断），结果决定走代理还是DNS only；随后完成80→443与生产CORS；②境内访问风险**需重新定性**：交接材料记录的真实现象是"境外未备案域名+境内SNI干扰"导致间歇性`ERR_CONNECTION_RESET`，这与"跨境线路质量"是不同问题、处置方式也不同，且知了Hub已通过Cloudflare代理缓解，不能继续按线路问题评估；③定时备份、异地副本和真实破坏恢复演练；④运维自动化三缺口——Linux服务器一键启停/健康验收、镜像registry发布、按精确标签的无损升级/回滚自动化；⑤9项低优先遗留编号`L9/F14/F21/F22/F24/F38/F39/F44/F48`；⑥清理源码中过时F编号依据注释；⑦评估developer只读统一配置快照机制。网页版剩余批次和Phase C继续按用户后续时机安排，不混入当前Phase B |
...
| 生产配置与密钥注入 | 后端`.env.example`现有58项：完整覆盖`config.py`读取的57项运行配置，并额外声明备份脚本使用的`BACKUP_ENCRYPTION_KEY`；只允许真实默认参数、格式说明和`CHANGE_ME_*`占位符。当前本地与服务器Compose都从后端工作树内、被`.gitignore`排除的`.env`通过`env_file.path + format: raw`注入，`.dockerignore`再保证它不进入镜像；这是逻辑隔离，不是物理目录隔离。Phase B必须在服务器现场创建实例独立配置，不得复制开发机`.env`。备份密钥不得复用或与备份包同失效域；生产CORS不得包含`null`。部署仓库自己的`.env`保存`SERVER_PUBLIC_IP`（多IP机器填写知天专属地址且不带端口）以及2026-08-16新增的`ZHITIAN_CUSTOMER_SERVER_NAME`、`ZHITIAN_ADMIN_SERVER_NAME`、`ZHITIAN_TLS_CERT_PATH`、`ZHITIAN_TLS_KEY_PATH`四项；这五项参与Compose插值（与后端`.env`的`format: raw`边界不同），模板中一律只留`CHANGE_ME_*` |
...
| Git 存档 | ✅ 最新标签组合为后端`zhitian v3.4`、部署仓库`zhitian-deploy v3.3`、管理后台`zhitian_admin v3.2`、客户端`zhitian_app v3.2`。源码版本分别为后端`3.4.0`、管理后台`3.2.0`、Flutter`3.2.0+320`；后端根路由和OpenAPI严格读取VERSION。既有`v3.3`已被生产服务器检出并被跨项目引用，禁止删除重打；服务器升级到v3.4必须显式检出新标签，不跟随默认分支。后端漏洞策略仍因F38及系统层无修复版本项红灯 |
