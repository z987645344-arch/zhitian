...
| 上一轮完成 | 2026-08-17 `docs/claude_skill.md`瘦身为执行agent视角（承接同日指挥师`v3.4.4`「存档职责划分补默认归属」）：**302行→184行**，移出指挥师职责、执行者选型、指令格式整章、0–9步工作流程与7条服务器红线（被删内容已完整移到仓库外`指挥师手册.md`，不是丢弃）；原第三章的「CHANGELOG条目格式规范」5条逐字搬到新**第八章**，新增**第九章存档操作规范**（对齐知了hub同名章节，补此前的规则单边缺失）。章节编号保留历史值，「二→四」空档刻意不重排、不复用旧号。六处「保留」段经SHA-256按行区间比对与`HEAD`**逐字一致**；唯一被修改的规则是作废「指令里没提的就是不动的」（改采禁区必须显式写出）。本仓库改动仅`CHANGELOG.md`与`docs/claude_skill.md`两个文件，零代码，**尚未提交**，计划`v3.4.5` |
| 当前等待 | **生产实例已于2026-08-13迁至染云数据香港（Ubuntu 24.04）；此前的腾讯云实例因跨境网络质量问题已退款作废，任何文档、脚本或操作都不应再指向它。** 四服务全部healthy；同机多IP入口与Compose `env_file.format: raw`加固统一归入后端/部署仓库`v3.3`。2026-08-16已通过SSH完成两项前置核对并**全部通过**：Compose实际版本`v5.4.0`（`format: raw`要求≥2.30.0）、后端`.env`共18个`KEY=value`且**零个值带引号、零个值含`$`**（核对只读取变量名，未读取任何值）。服务器当前检出后端与部署仓库`v3.3`、管理后台`v3.2`。**双子域名与源站HTTPS的仓库侧改动已于2026-08-16存档并推送**：部署仓库`v3.4`（提交`8585c7d`）、后端`v3.4.3`（提交`71a1882`），两个仓库的远程分支、标签与本地HEAD三者一致，工作区归零。**因此剩下的全部是服务器侧待办**：签发Origin CA证书、添加两条DNS记录、现场填写`.env`（四项`CHANGE_ME_*`真实值 + `ZHITIAN_FORCE_HTTPS=on`；私钥须`chown root:101`+`chmod 0640`，否则以uid 101运行的反代会因无权读取私钥启动失败）、重建反代容器、完成线上验证并把生产`CORS_ORIGINS`更新为正式域名。服务器当前仍检出`v3.3`，升级必须显式切换标签、不跟随默认分支。不得把本机验证冒充生产结果。网页版批次二剩余文件库、工具箱、欢迎页/附件展示等体验项根据后续指令安排；F44与F48继续保持P3待讨论 |
