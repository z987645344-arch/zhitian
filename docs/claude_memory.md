...
| 指挥师 | Claude Code（指挥师角色） | 阅读现状、与用户确认范围、拆解可执行指令、按任务性质选择agent、判断验证结果、维护协作文档 |
| 设计取向执行 | Codex | 更适合视觉/交互、信息架构、方案探索及需要设计判断的实现；仍须按明确范围执行和验证 |
| 严格执行取向 | Claude Code（执行者角色） | 更适合按明确规格编程、跨文件同步、部署操作、GitHub操作和确定性验证；与指挥师角色必须在任务中明确区分 |
...
| 上一轮完成 | 2026-08-15完成交接文档收尾：工作手册与本文件改为Claude Code指挥、按任务性质调度Codex/Claude Code执行；移除WorkBuddy作为当前流程角色，保留真实技术历史；README与部署指南同步校准当前版本及服务器阶段措辞 |
| 当前等待 | 腾讯云Phase B实例已运行四服务；同机多IP入口与Compose `env_file.format: raw`加固统一归入后端/部署仓库`v3.3`。当前任务未附加SSH终端，尚待在服务器核对Compose真实版本、后端`.env`是否全部为无引号`KEY=value`，随后才能执行保卷`down/up`及ready/管理后台/customer浏览器回归；不得把本机验证冒充生产结果。网页版批次二剩余文件库、工具箱、欢迎页/附件展示等体验项根据后续指令安排；F44与F48继续保持P3待讨论 |
...
| 下一步 | 当前已知待办汇总：①Phase B正式域名、HTTPS、80→443、生产CORS与服务器加固；②评估境内访问香港实例的跨境线路不稳定风险，需结合真实境内运营商去程而非只看服务器回程；③定时备份、异地副本和真实破坏恢复演练；④运维自动化三缺口——Linux服务器一键启停/健康验收、镜像registry发布、按精确标签的无损升级/回滚自动化；⑤9项低优先遗留编号`L9/F14/F21/F22/F24/F38/F39/F44/F48`；⑥清理源码中过时F编号依据注释；⑦评估developer只读统一配置快照机制。网页版剩余批次和Phase C继续按用户后续时机安排，不混入当前Phase B |
...
| 生产部署 | 后端和管理后台历史生产镜像已在Docker Desktop 29.6.2+WSL2真实构建；独立私有仓库`z987645344-arch/zhitian-deploy`中的Compose已真实验证同源`/api`转发、具名卷、tmpfs、日志轮转、重启与资源限制。腾讯云Phase B实例当前四服务可用；2026-08-13因同机知了Hub需要使用另一公网IP的80端口，源码把知天反代发布地址由通配80收窄为`${SERVER_PUBLIC_IP}:80`，真实值只进入未跟踪`.env`，静态插值验证通过，生产重启与监听核验仍等待SSH会话。2026-08-01锁定`numpy==1.26.4`后的干净镜像已通过启动、`/ready`和Chroma读写；正式域名/HTTPS、异地备份与系统加固仍待继续完成 |
