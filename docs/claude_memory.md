...
> **最后更新：2026-08-09**（网页版工作台正式建设批次一完成：会话侧栏、历史恢复/删除、session持久化与fast/expert切换均已接入既有接口；真实Compose浏览器链路与383项权威回归通过。后续文件库、工具箱和设置页留给批次二/三）
...
| 上一轮完成 | 2026-08-09完成网页版工作台正式建设批次一：`web_client`新增会话侧栏、新建/恢复/删除、localStorage会话持久化及fast/expert切换，后端会话接口和SSE契约保持不变。真实浏览器完成登录、双模式发送、刷新恢复、历史切换、删除、附件上传和退出；同时修复静态JS旧缓存与fast `[DONE]`早于会话落库的两处真实边界。临时账号/3个剩余会话/1个附件已精确清理，完整权威回归`383 passed, 5 deselected` |
| 当前等待 | 云服务器正在办理；本机Flutter客户端继续按`http://localhost/api`走Compose环境人工验收。网页版正式建设三批中的批次一已完成，文件库管理、工具箱与设置页仍明确留给批次二/三，不应回填进本批。知识库核验规则现由动态规范模块统一承载；F44仍为P3性能体验待办，等待后续决定是否优化“本地证据充分时跳过联网搜索”。0号接管仍按既定部署脚本和安全边界处理 |
...
| 下一步 | 当前实施主线转入Phase B服务器落地：系统加固、DNS/HTTPS、生产密钥与CORS收紧、反向代理下SSE/限流、真实业务链路和服务器备份恢复演练。网页版后续批次按用户单独指令推进：批次二/三再补文件库、工具箱与设置页；批次一不扩项。代码侧继续观察F38上游约束，F39保持P3待修，F44等待讨论是否优化“本地证据充分时跳过联网搜索”。白标外售/二创仍只归档在Phase C，不占用当前资源 |
...
| F36 | ✅ 文档上传与知识录入已异步任务化：立即返回task_id，`GET /tasks/{id}/stream`提供SSE进度和心跳、查询端点兜底；任务仅允许owner读取，启动时中断旧任务并清理SQLite/Chroma半成品，失败按Level1重试，组织内按内容哈希去重。上传仍以1MB与2000切片为安全上限 | main.py / layers/task_store.py / zhitian_admin | 已完成（异步任务化+SSE进度反馈，2026-08-08） |
| F37 | ✅ 已切换到MIT许可的`BAAI/bge-small-zh-v1.5`自研ONNX实现，512维、max length 512、阈值0.55；代码已合并master，109条存量向量于2026-08-07完成迁移并核对数量/id/doc_id覆盖，Compose于2026-08-08重建后中文三问全中、无关问题全拒答。旧384维回滚库在确认加密备份独立可恢复后已删除；Flutter端1MB上限也于2026-08-09合并并重建安装包 | layers/embedding.py / scripts/migrate_embeddings.py / Dockerfile | ✅ 已解决 |
...
- 网页版工作台正式建设分三批：2026-08-09已完成批次一（会话侧栏、历史持久化、fast/expert）；文件库管理、工具箱和设置页留待批次二/三由用户另行启动，不在批次一顺手扩展。
...
| customer网页客户端 | **`web_client/`是后端仓库内的纯静态customer前台，无框架、零构建、零运行时依赖**：`login.html`/`register.html`/`chat.html` + `config.js` + `css/style.css` + `js/{api,login,register,chat}.js`。2026-08-09正式工作台批次一已取代原“单会话、仅fast接口验证壳”：左栏复用`GET /memory/sessions`、`GET /memory/{session_id}`、`DELETE /memory/sessions/{session_id}`实现新建、倒序历史、刷新恢复、切换和二次确认删除；当前`session_id`与fast/expert偏好存localStorage，显式退出/401自动登出统一清会话指针，恢复404也放弃失效指针，避免账号切换复用旧标识。`/chat`和`/chat/stream`均按当前模式显式传参，expert有长等待说明与发送期防切换，原SSE解析继续复用。**权限范围仍严格限customer**：不调用管理接口、不含知识库录入；聊天附件是customer原有`/chat/attachments`权限，不是本批放权。后端owner校验使他人会话统一不可见。历史接口目前只持久化消息正文和附件名称，citations/reasoning不会在刷新后重现，与Flutter现有恢复能力一致。HTML不缓存、JS/CSS缓存1小时，因此工作台资源使用`?v=workspace-b1-2`保证无构建发布时同批加载；fast流会先发`[DONE]`再绑定会话，前端用`0/160/520ms`三次有限确认解决列表即时刷新竞态，不做常驻轮询。地址仍由`config.js`默认同源`/api`，token沿用`zt_web_*` localStorage；**已知XSS取舍**仍是改HttpOnly Cookie需后端Cookie签发与CSRF配套。附件上下文走`_answer_from_supplied_context`时不产生citations，属既有后端行为。容器继续使用非root `nginx:stable-alpine`、严格CSP与仅内部frontend网络；本地Compose经`/customer/`前缀转发，Phase B改子域名时替换代理规则即可。后续文件库、工具箱、设置页明确属于工作台批次二/三，批次一不包含。涉及目录`web_client/` |
