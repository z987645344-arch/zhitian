...
> **最后更新：2026-08-17**（存档职责划分补默认归属与「规则须写进两份手册」；此前：协作架构切换为Claude Code指挥，按任务性质调度Codex或Claude Code执行，云服务器现场操作由指挥师独占执行）
...
| **推送执行** | **默认由执行 agent 完成**，与本章开头「Claude Code 执行者更适合GitHub操作」的取向一致。用户可以逐次改派，但**必须是明确指定执行者**；用户对**批次或时机**的回答（例如“一起存档吧”——那回答的是三次授权是否合并）**不构成改派**，指挥师不得据此自行动手。无论最终交给谁，**提交、推送、打标签仍是三次独立授权**，任何一方都不得自主 commit |
