...
def list_document_chunks() -> list[dict]:
    """读取文档知识库全部chunk，供审核员查看知识资产。"""
    collection = _get_document_collection()
    try:
        result = collection.get(include=["documents", "metadatas"])
    except Exception as e:
        logger.error("Chroma文档chunk读取失败：error_type=%s", type(e).__name__)
        raise
...
def list_memory_fragments() -> list[dict]:
    """读取全部长期记忆片段，session_id只在接口层脱敏展示。"""
    collection = _get_chroma_collection()
    try:
        result = collection.get(include=["documents", "metadatas"])
    except Exception as e:
        logger.error("Chroma记忆片段读取失败：error_type=%s", type(e).__name__)
        raise
    ...
    for doc, metadata in zip(result.get("documents", []), result.get("metadatas", [])):
        ...
        fragments.append({
            ...
            "importance": str(metadata.get("importance", ""))
        })
    return fragments


def clear_all_vector_memory() -> int:
    """清空zhitian_memory集合，返回删除数量。"""
    collection = _get_chroma_collection()
    return _delete_all_from_collection(collection, "Chroma全部长期记忆清空失败")


def clear_all_documents() -> int:
    """清空zhitian_documents集合，返回删除数量。"""
    collection = _get_document_collection()
    return _delete_all_from_collection(collection, "Chroma全部文档知识清空失败")


def delete_memory_by_session_prefix(session_prefix: str) -> int:
    """按session_id前缀删除长期记忆片段。"""
    ...
    try:
        ...
        logger.error("Chroma按session前缀删除失败：prefix_len=%s error_type=%s", len(session_prefix), type(e).__name__)
        raise


def _delete_all_from_collection(collection, error_message: str) -> int:
    """按ids清空Chroma集合，兼容空集合。"""
    try:
        result = collection.get(include=["metadatas"])
        ids = result.get("ids", [])
        if ids:
        ...
        logger.error("%s：error_type=%s", error_message, type(e).__name__)
        raise
