...
FAST_TOOLS = [
    {
        ...
        "function": {
            "name": "search_documents",
            "description": (
                "仅检索本地已审核企业知识库内容。判断是否调用本工具的标准，是用户问题是否可能需要"
                "企业知识库核验，而不是你是否凭通用知识已经会回答；不要因为自己知道一个概念就跳过检索。"
                "用户不必显式提到‘文档’或‘知识库’。当问题涉及术语定义、概念解释、产品、项目、编号，"
                "或者可能属于企业资料的事实性、规范性、依据性内容时，应优先调用本工具核验；当问题与"
                "当前知识库覆盖的专业或业务领域相符时同样如此。只有明显的问候、寒暄、开放闲聊、个人偏好，"
                "或明确不需要企业资料核验的问题才直接回答，不能对所有问题一律检索。"
                "不要用于互联网新闻、天气、价格或最新消息。"
            ),
            "parameters": {
                "type": "object",
                "properties": {
...
def _build_fast_messages(state: AgentState) -> list[dict]:
    fixed_prompt = system_modules.prompt_prefix(
        ...
        "工具选择依据是问题是否可能需要企业知识库核验，不是你凭通用知识能否直接作答。"
        "对可能属于当前知识库专业或业务领域的术语定义、概念解释、事实性、规范性或依据性问题，"
        "优先调用search_documents；用户不需要显式提到文档。需要列出文件清单时调用list_documents。"
        "明显的问候、寒暄、开放闲聊和个人偏好直接回答，不要为了使用工具而过度检索。"
